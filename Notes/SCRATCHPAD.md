...
- **NEXT (Ready flip authorized and imminent; the rest are separate author acts):** flip PR #24 to Ready, then STOP; merge; after merge issue a byte-exact launch authorization naming `<M56>`; user executes the Della run; separately commit evidence; separately append v1.22. Superseded-anchor deletion remains OPEN.
