...
- **Review gate DONE.** Full suite at the implementation head `099cd728`: 1228 passed / 2 skipped / 0 failed. ONE fresh Codex gpt-5.6-sol xHigh (session `019f90b0`, read-only, banner + rollout verified) exact-head review returned **REVISE** (2 BLOCKER / 2 MAJOR / 2 MINOR / 1 NOTE), all orchestrator-confirmed; the SINGLE bounded correction pass closed all seven: literal-40-hex RUN template + frozen no-clobber `sacct` capture (`runs/poster_d58/job_metadata.txt`); render now VALIDATES the saved grid and ENFORCES the training boundary (xlim clamp per figure's units + "forecast"/"train" annotation strip); `atomic_savez` makes both NPZ writes atomic; closed-world `verify_run_dir` with duplicate/miscount manifest rejection; A7 caption half moved to a POST obligation; help-text fix; tests parameterized + AST call-order/alias-loader guards + derivation/boundary/field-order/effective-Slurm-command pins (focused 62 passed). Correction pass SPENT; fresh exact-head re-review before Ready available at author option (D56d precedent).
- **Draft PR #32 OPEN; confirmation round + FINAL correction pass (2026-07-23).** Author-cast exact-head confirmation at `a3d992d8` (fresh Codex gpt-5.6-sol xHigh read-only, session `019f91a9`) returned **FINDINGS**: B1/B2/M1/seal/allowlist/tail CONFIRMED-CLOSED or PASS, but F1 MAJOR (P4 lost its terminal-state gate; CWD-relative redirect), F2 MINOR (`figures` exemption name-based, not type-checked), F3 MINOR (five surviving mutants), F4 NOTE (`.err` is not gitignored); all orchestrator-confirmed, returned to author. Author adopted **option (a): SECOND AND FINAL bounded pass** (six-file surface; Slurm/fit-spec/schema/policy frozen). Implementer Codex gpt-5.6-sol xHigh workspace-write session `019f91d5` closed F1-F4 (P4a readiness gate + P4b absolute-anchor capture; figures type check; five discriminating tests; precise gitignore wording); orchestrator read every changed line and ran the mutation battery — six mutants KILLED (M1-M4, M5a retry-after-`||`, M5b second-sbatch), byte-identical restores sha256-verified, control green. Focused 79 (53 driver + 13 argparse-guard + 13 provenance, per `pytest --collect-only`; an earlier version of this line and D58 Update 2 misattributed the split as 52+13+14 — corrected by the authorized Notes-only truth commit, D58 Update 3); **full suite 1250 passed / 2 skipped / 0 failed** (+4 = the new tests). FINAL pass SPENT. **Final confirmation DONE at exact head `e274b36f`** (fresh Codex gpt-5.6-sol xHigh read-only, session `019f91e5`, rollout-verified): **F1-F4 CONFIRMED-CLOSED**, byte-freeze + launch path PASS, zero surviving mutants, ONE new MINOR = the focused-split misattribution itself (orchestrator-confirmed via collect-only; zero disagreement, GLM not consulted). Author adjudicated it record-accuracy-only and authorized this single Notes-only truth correction (NOT a third technical pass); no technical byte changed after `e274b36f`; the Notes-only final head is NOT re-reviewed. Branch pushed with `e274b36f` + this tail; PR #32 body carries the full lineage. **HARD STOP before Ready/merge/Della.** D58-RUN needs a fresh byte-exact authorization naming the LITERAL M58 sha40 (protocol §7.3); D58-POST acts are separate. M2cR STOP + D45 untouched; paper-grade D19 resumption is a later separate act.
...
- **Pin.** Threads = 3 for D19/A7-governed `bistar_gp` CPU workloads on the frozen `intel,cascade,rh9` 90-node Della pool; four environment variables + torch intra-op consistently 3, `cpus-per-task=3`; no claim outside the D19/A7 scope.
- **Headline.** Whole-cell elapsed (RECORDED seconds, rounded here): sub t1–t4 `34.59 / 24.87 / 24.47 / 24.18`; full `64.41 / 37.76 / 33.98 / 42.34`. Full-t4 prior-eval regression (DERIVED ms) `17.85` vs t3 `9.48`.
- **Budgets.** All frozen ceilings fit at pin 3; binding paper-target row retains **1.67x** DERIVED headroom under its own x1.5 engineering-overhead convention.
- **Allowlist.** Exactly `docs/prereg-addenda-d19.md`, `Notes/DECISIONS.md`, `Notes/SCRATCHPAD.md`, `Notes/CHATLOG.md`.
- **Workflow.** Implementation done (head `145a4e5`; implementer Codex gpt-5.6-sol xHigh session `019f8f82`, rollout-verified). Review gate DONE: fresh read-only Codex gpt-5.6-sol xHigh (session `019f8f90`) at exact head `145a4e5` returned **APPROVE, zero findings** (88/88 recorded cells, all derived scalars recomputed clean); orchestrator agreed; GLM 5.2 not consulted; the single bounded correction pass is UNSPENT. **R57 = `145a4e5`**; this alignment is part of the identified Notes-only tail. Push + **Draft PR #31** DONE (the earlier in-session push denial was transient; the plain-form push succeeded under the standing authorization). **STOP before Ready**; Ready/merge producing M57 is a separate author act.
- **NOT authorized.** Ready/merge; Della contact or benchmark rerun; non-allowlist edits; scientific gate/threshold/scale/candidate/holdout changes; poster work; Mauna fitting or other science. Poster milestone is **D58**.

## A7 job-11517022 evidence closure — validated timing evidence at canonical path, original incomplete capture preserved, recovery record added; E57/N57 topology; PR #30 MERGED; evidence-closure merge anchor 24f55c37ecd2de1ef7f2f1533c18553f6a8722d8 = origin/main (true merge; branch preserved) (2026-07-23) — branch `evidence/d19-a7-timing-11517022` off M56d `5fcc2d316f6894ed793418d9ab16274e6c8a1ad2`
