...
- **Plan:** K (this: capture+audit+tests+`git rm` packet-03 + D52 U10; ledger/evidence EXCLUDED) → regen T-B → L (self-consistent tree, unflagged pytest green; flag only in the regen window) → E3-retrievability + chain-separation proofs → ONE Codex xHigh review → fixed auditor over preserved evidence + 1503-file byte-identity re-hash + derive closure → commit closure+evidence + Draft PR → STOP. E3 `367667f` immutable (chain `d8a3f302…`); regenerated manifests are the CURRENT tree, NOT the `-03` chain. Packet-03 preserved at G3 `5cb3527` blob `f7fa1a02` (sha `54e28dcf`); HEAD has no executable packet.
