...
- **D56a (docs-only) —** Supersedes the self-blocking broad §7 diff with the explicit two-anchor rule: execution bytes close to `H'`, the known guard-only test delta is blob-pinned, and the protocol closes to reviewed amendment head `A` at launch anchor `M56a`. Three-file allowlist only; Draft PR pending; **STOP before Ready / merge / launch**.
- **NEXT:** merge PR #24 is DONE at `66ca91cd`; next acts are D56a Ready/merge (separate author acts), then the byte-exact launch authorization naming `M56a` under the amended §7 rule.
