...
## D56 — A7 execution-protocol freeze; Draft PR pending, STOP before Ready/merge/launch (2026-07-21) — branch feat/d19-a7-d56-execution-protocol off origin/main 3b44ec2b
...
- **Files.** NEW `experiments/submit_d19_a7_bench.slurm`, `experiments/d19_a7_validate.py`, `tests/test_d19_a7_protocol.py`, `docs/d19-a7-execution-protocol.md`; APPEND/UPDATE Notes only. Script owns preflight + serial STOP matrix; validator owns V1–V15 + census + frozen-anchor rehash.
- **Verification.** `tests/test_d19_a7_protocol.py`: **22 passed**; frozen `tests/test_d19_bench_firewall.py`: **68 passed**. Both hermetic; `d19_bench.py`, its firewall test, and `runs/` untouched.
- **NEXT (each a separate author act; none authorized):** decide Draft→Ready and merge; after merge issue a byte-exact launch authorization naming `<M56>`; user executes the Della run; separately commit evidence; separately append v1.22. **STOP before all of these.** Superseded-anchor deletion remains OPEN.
