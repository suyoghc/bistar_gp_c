...
- **NEXT (each a separate author act; none authorized):** flip Draft PR to Ready, merge, then the A7 timing run (local, then thread-pinned on Della) and the thread-pinning addendum. Deleting the two superseded `runs/d19_planning/bench_*.json` artifacts is an OPEN separate author decision. **STOP before all of these.**
