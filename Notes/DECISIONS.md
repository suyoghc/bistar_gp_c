...
**Boundaries at the final head:** the diff vs `origin/main` is exactly the two new files, the four regenerated artifacts, the fixed template, the report-currency lines, and Notes (D51 + SCRATCHPAD + CHATLOG); no R2/R2a module, R1 schema, v1.17/v1.20/v1.21 artifact, or frozen-gate byte changed; the ledger is untouched (one D45 line); `docs/m2c_evidence/` stays absent; no `--execute` ran; no scientific, model, or diagnostic computation ran anywhere in the session (hermetic fakes, read-only git, and hashing only). **R4 remains BLOCKED and NOT begun**: it still requires the author-authorized preparation (T-B freeze regeneration, committed launch packet, prospective grant) and the separate one-shot launch authorization, all AFTER this PR merges. D45 stays permanently UNVALIDATED_ATTEMPT; the v1.18 instance path stays ABSENT. **Draft PR opened; NOT flipped Ready; NOT merged. HARD STOP.**
