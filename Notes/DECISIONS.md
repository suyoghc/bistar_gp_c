...
## D51: M2cR R4 preflight BLOCKED (no committed production launch vehicle); author ballot resolutions; narrow hermetic R3a amendment authorized — generic launcher + launch-packet contract + fixed diagnostic template — 2026-07-19

**Problem:** The author-directed read-only R4 preflight (2026-07-19) at `origin/main` = `b19cce2` (PR #18 merge, 2026-07-19T00:16:32Z; parents `35ccc3d` + audited `70e8172`) PASSED the startup gate and the full authority verification: tracked tree byte-identical to `origin/main`; v1.18 instance absent; ledger = exactly the one D45 historical line; no `docs/m2c_evidence/`; every infrastructure pin (8 artifacts, 2 R1 schemas, 12 code files) byte-verified; protocol-manifest pins and Layer-1a reference verified (manifest `5afb0f00…`, schema `076cdc2d…`, parameters `97625715…`, infra `b63be66b…`); v1.17 canonical recomputed `65381bc7…e522e2` via the committed serializer; all 154 worktree-root importable-manifest entries match the merged tree with zero missing and zero unlisted files. It then classified R4 **BLOCKED** on one gap: no committed, reviewed argv-level production launch vehicle exists. `launch_config_from_freeze` (`bistar_gp/m2cr/capture.py:3553`) and `capture_run` (`capture.py:2088`) are the reviewed factory/driver pair, but `capture.py` has no `__main__`, `pyproject.toml` declares no console script, and `tests/test_m2cr_launch_authority.py` explicitly defers production-path proof to the pytest-invoked real-root battery. The R4 contract prohibits ad hoc `python -c`, scratch runners, and session wrappers (the D45 unreviewed-wrapper failure class), and R4 is execution-only and may not introduce implementation.

**Decision (author ballot, cast 2026-07-19):**

- (a) T-B freeze regeneration at a future R4 preparation: ADOPTED. (b) R4-created launcher: REJECTED — creating `experiments/m2cr_r4_launch.py` inside R4 would be new execution implementation and would violate the preflight contract; **R4 remains BLOCKED before grant or launch**. (c) `one_shot: true`: ADOPTED. (d) Grant date: the actual local date when a future grant is appended. (e) Draft evidence PR: ADOPTED. (f) Capture-at-source evidence directly under `docs/m2c_evidence/<run_id>/`: ADOPTED.
- **One narrow hermetic R3a amendment AUTHORIZED** (infrastructure only; authorizes no R4 grant and no execution): a committed generic production launcher `bistar_gp/m2cr/r4_launch.py`, runnable as `python -m bistar_gp.m2cr.r4_launch`, consuming a **closed-world canonical machine launch packet** (`kind: "m2cr_launch_packet"`, `schema_version: 1`) that binds the exact execution commit, the complete six-member diagnostic chain, the prospective authorization id, the launch-attempt and run ids, the committed bootstrap-template path + sha256, the authenticated manifest paths/digests, the detached-worktree identity, the final evidence directory, and the frozen 8-hour ceiling — never embedding R4-specific ids or hashes in the launcher itself. An explicit `--execute` flag is required; without it the launcher validates and reports only and must never append an attempt, spawn, create an evidence directory, or call `capture_run`. On `--execute` it validates the committed prospective grant and requires it unconsumed (`verify_chain(..., require_unconsumed=True)` plus an explicit grant-`frozen_chain`-equals-packet-chain check), verifies the packet, template hash, effective chain, execution commit, and detached worktree, then invokes **only** `launch_config_from_freeze` followed by `capture_run`, appending the schema-valid `launch_attempt_started` line to the working-tree ledger between factory success and `capture_run` (a factory refusal starts no attempt; once `capture_run` runs, a terminal record always exists to close the attempt; committed ledger states stay stream-valid because closure events land in the same post-run commit). No wrappers, monkeypatches, observers, probes, or post-hoc transcription; `capture.py` stays the sole execution/capture authority. The fixed diagnostic template is committed at `docs/m2c_freeze/m2cr_r4_bootstrap_template_v1.json` (canonical bytes, sha256 `d62fee603088979111ff6f63c66c29093a4b3f413ee1d52c2f4869f17493b299`): payload entry `bistar_gp.m2cr.diagnostic_payload:diagnostic_payload_entry`, `pass_context: true`, nothing else.
- **Constraints:** the launcher takes NO infrastructure-manifest code pin (D50 ballot-C precedent: post-R2 modules are bound by `execution_commit` plus the importable-artifact manifest with origin/loader authentication; audit's `_R2_CODE_RELPATHS` exact-set check stays untouched). No R2/R2a module edits; any need for a new scientific/protocol decision or a change to the ratified chain, ledger semantics, capture boundary, or v1.21 rules STOPS the work and returns the decision to the author. R4-specific packets, grants, attempt ids, and evidence stay OUT of R3a. **R4 is not begun**: no ledger event, no R4 branch or freeze regeneration, no `docs/m2c_evidence/`, no `--execute`, no diagnostic/model computation. D45 remains permanently UNVALIDATED_ATTEMPT; the v1.18 instance path stays ABSENT.

**Alternatives considered:** an R4-created launcher (rejected by ballot — implementation inside an execution-only milestone); modifying `capture.py`/`bootstrap.py` to add a CLI (rejected — source inspection proves a separate module suffices: the factory/driver pair and every needed validator are importable API, so amending infrastructure-pinned R2 modules is unnecessary); an uncommitted one-shot invocation (rejected — the D45 unreviewed-wrapper class); a new committed JSON-Schema file for the packet (rejected — the packet is operational input validated closed-world in code with hermetic tests, matching the R2 freeze-artifact precedent; schema files stay reserved for record/ledger contracts per B3).

**Workflow (author-directed):** branch `feat/d19-m2cr-r3a-launch-vehicle` off `origin/main` `b19cce2`; docs-first commit (this entry + SCRATCHPAD + the template); bounded Codex gpt-5.6-sol xHigh implementation over exactly {`bistar_gp/m2cr/r4_launch.py`, `tests/test_m2cr_r4_launch.py`}; Fable reads and verifies every change; regeneration by the established fresh-detached-worktree recipe with the expected pin cascade (importable manifest, environment-freeze, infrastructure, protocol manifests; report-currency update per the F8 cross-check); focused tests then `python -m pytest -q`; ONE bounded read-only Opus 4.8 exact-head review (production launcher, packet authority, no-execute boundary, R3/R4 separation), every finding independently verified by Fable, at most one consolidated correction pass, no multi-model panel; push + Draft PR; HARD STOP before Ready, merge, any R4 regeneration or grant, `launch_attempt_started`, `--execute`, evidence creation, or scientific/diagnostic computation.

**Status:** IN PROGRESS — completion will be appended here.

### D51 — Update 1 (2026-07-19): R3a implemented, regenerated, reviewed; Draft PR opened; HARD STOP before Ready/merge

**Commits** (branch `feat/d19-m2cr-r3a-launch-vehicle` off `origin/main` `b19cce2`): `d0a317d` (docs-first: D51 + SCRATCHPAD + the fixed template, sha256 `d62fee60…`) · `c11db47` (implementation: `bistar_gp/m2cr/r4_launch.py` + `tests/test_m2cr_r4_launch.py`) · `601f599` (regen at `c11db47` + Layer-1b re-pin + report currency) · final docs commit (this update).

**Implementation, exactly the D51 scope:** `bistar_gp/m2cr/r4_launch.py`: closed-world canonical launch packet (`kind: "m2cr_launch_packet"`, `schema_version: 1`, a 14-key exact set) binding the execution commit, the six-member diagnostic chain, the prospective authorization id, the attempt/run ids, the template path+sha256, the three manifest paths/digests, the detached-worktree identity, the derived evidence directory (`docs/m2c_evidence/<run_id>/`), and the frozen 8 h ceiling; validate mode is read-only (rc 0/1, one canonical JSON report line); `--execute` first re-validates everything — packet canonicality and closed world; packet/template/manifests committed at HEAD; template equals the fixed diagnostic document; `validate_ledger`; `verify_chain(require_unconsumed=True)` PLUS the stronger grant-`frozen_chain`-equals-packet-chain equality; execution-commit resolvability; attempt-id freshness; evidence-dir absence; worktree existence/distinctness/detached-HEAD-equals-commit/tracked-clean/pin digests — then invokes ONLY `launch_config_from_freeze` followed by `capture_run`, appending the schema-validated `launch_attempt_started` line between factory success and capture (a factory refusal is rc 2 and appends nothing); capture publication exceptions propagate after a truthful report; the post-run report states marker existence, consumption (definitionally the marker), the frozen terminal status, `not_a_result`, payload presence, and the terminal/raw digests. One recorded deviation from the implementation brief, verified sound by Fable: the repo-root authority derives from the packet's own committed git toplevel (production = this repository; hermetic tests = tmp repositories) rather than from the module location. The launcher takes NO infrastructure code pin (ballot-C precedent); `_R2_CODE_RELPATHS` is untouched.

**Tests:** 48 hermetic tests in `tests/test_m2cr_r4_launch.py`: constants-vs-frozen-authorities cross-checks (pattern strings byte-equal to the R1 ledger schema; relpaths equal to capture's; template bytes + golden sha `d62fee60…`); validate-mode zero-side-effect proofs with fail-if-called spies; the positive `--execute` path (factory + capture exactly once, one canonical attempt line `m2cr-ev-000002` present before capture runs, exact factory arguments); factory-refusal and validation-failure no-append proofs; 20 parametrized packet-document rejections; noncanonical-committed, uncommitted-packet/template/ledger, and the template-substitution three-way discrimination (disk digest vs fixed document vs HEAD); historical-grant, grant-digest-mismatch (proving the equality check is load-bearing beyond `verify_chain`), grant-commit-mismatch, consumed-grant, and stale-attempt-id rejections; evidence-dir blocker naming; five worktree rejections; event-id and line-schema units; publication-exception propagation.

**Regeneration** (`601f599`, established fresh-detached-worktree recipe at `c11db47`): importable manifest 8,746,622 B / 39,968 entries (+2 sources: the launcher and its test file; 0 orphans; all 156 worktree entries byte-verified against the tree with zero unlisted importables); environment-freeze `1e817d6e…`; infrastructure `b1c8a817…` (code section unchanged: the 12 R2 pins); protocol re-pin `d3ddebee…` (schema pin `076cdc2d…` and parameters pin `97625715…` unchanged); interpreter pin, child-env mapping, dependency lock, native-stack expectations, and preboundary set byte-identical; `verify_infrastructure_manifest`, `verify_evidence_ceiling_compliance`, `verify_environment_freeze`, and `verify_protocol_manifest` all OK; fixed-artifact total 8,870,677 B with the report updated truthfully (F8 cross-check green).

**Suite:** focused launcher + audit suites 100 passed; manifest/measure suites 76 passed / 1 skipped; full `python -m pytest -q` at `601f599`: **1005 passed / 2 skipped / 0 failed** (6:41; both skips are the pre-existing opt-ins), including the four real-root integration launches against the regenerated committed tree.

**Review gate:** bounded Codex gpt-5.6-sol xHigh implemented under the D51 brief; Fable read and verified every line pre-commit. ONE bounded read-only Opus 4.8 exact-head review at `601f599`: **APPROVE — zero BLOCKER/MAJOR/MINOR; two advisory NOTEs**, each independently verified by Fable against source and adopted as recorded facts rather than defects: (1) on `--execute`, rc 0 / report `ok: true` certify the launch MECHANISM (a durably published terminal record), never the diagnostic outcome — the frozen status travels in `report.terminal_status`, and the launcher deliberately writes no ledger closure events (those belong to the post-run R4 commit, keeping every committed ledger state stream-valid); (2) `worktree_clean` is tracked-only by design — untracked worktree files cannot affect certification because capture authenticates every consumed artifact by digest and the child's importable-manifest pre-walk rejects any added importable artifact. No correction pass was needed.

**Boundaries at the final head:** the diff vs `origin/main` is exactly the two new files, the four regenerated artifacts, the fixed template, the report-currency lines, and Notes (D51 + SCRATCHPAD + CHATLOG); no R2/R2a module, R1 schema, v1.17/v1.20/v1.21 artifact, or frozen-gate byte changed; the ledger is untouched (one D45 line; no grant or launch-attempt event was recorded); `docs/m2c_evidence/` stays absent; no `--execute` ran. **Computation provenance, stated precisely (wording corrected by author direction, 2026-07-19):** No R4 payload or real scientific model/profile diagnostic execution ran; `_real_components` was never executed; no Mauna/holdout computation ran. The full hermetic suite exercised the launcher with fakes/spies, diagnostic/classifier machinery on synthetic inputs, four real-root infrastructure launches, and the repository's pre-existing hermetic tiny-E1 sampler regressions. **R4 remains BLOCKED and NOT begun**: it still requires the author-authorized preparation (T-B freeze regeneration, committed launch packet, prospective grant) and the separate one-shot launch authorization, all AFTER this PR merges. D45 stays permanently UNVALIDATED_ATTEMPT; the v1.18 instance path stays ABSENT. **Draft PR opened; NOT flipped Ready; NOT merged. HARD STOP.**

## D52: M2cR R4 preparation — T-B freeze regeneration, canonical launch packet, prospective one-shot grant; author rulings A1 (closure acceptance stack), B (standalone caffeinate), C (stale worktree untouched); execution NOT begun — 2026-07-19

**Problem:** PR #19 merged R3a at `66bba025bc11b51e4cae126f974e758dab31be8f` (2026-07-19T09:51:20Z), providing the committed production launch vehicle whose absence had blocked R4 (D51). The author-directed read-only R4 preflight (2026-07-19, sole-orchestrator session) then PASSED the startup gate (tracked tree byte-identical to the merge; ledger exactly the one D45 historical line `m2cr-ev-000001`; `docs/m2c_evidence/` absent; v1.18 instance absent; every launcher/template/manifest byte at the audited D51 values; v1.17 canonical recomputed `65381bc7…e522e2` stdlib-only; all 326 preboundary/native-stack pins and the interpreter pin rehash-equal) and returned: launch path PASS (the merged `bistar_gp.m2cr.r4_launch` provides the complete reviewed end-to-end path); closure path conditional on one author ruling (no committed closure-writer program exists; the reviewed fail-closed acceptance machinery — `validate_ledger`, `verify_ledger_against_evidence`, `verify_chain`, the frozen ledger schema, and the committed exemplar `tests/test_m2cr_audit.py::_valid_events` — does); and one material host fact: `pmset` shows AC-power sleep = 1 minute, so an unattended 8 h run requires an authorized sleep inhibitor. The author cast the R4 PREPARATION AUTHORIZATION (2026-07-19).

**Decision (author authorization and rulings, 2026-07-19):**

- **Ruling A — closure procedure, option A1 ADOPTED.** The post-run closure events may be assembled mechanically from the captured files, because no scientific value or status is chosen manually and the closure is ineligible for commit unless ALL of the following independently pass: the frozen ledger schema; `validate_ledger`; `verify_ledger_against_evidence` against the exact evidence directory; `verify_chain`; terminal-record schema validation; raw-manifest and per-file digest verification. The terminal status must be copied exactly from `terminal_record.json`; marker presence alone determines the consumption branch; all hashes are recomputed from files. Any disagreement means STOP with no closure commit. The ruling does not permit reconstruction or transcription of runtime evidence and does not authorize R5 interpretation.
- **Ruling B — host liveness.** A separate standalone `/usr/bin/caffeinate -i` process is authorized at launch time: started before the launcher, kept separate from it, terminated after the launcher exits; its absolute path, PID, start time, and stop time are recorded factually in this entry's launch update. It must not wrap, instrument, inspect, or alter the launcher or the frozen child environment. No `-w`, no shell wrapper, no `pmset` change. (Not started during preparation.)
- **Ruling C.** The stale R2-era registered worktree (`…/67dd9d0a…/scratchpad/r2-detached-worktree` at `6a90522`) stays untouched.
- **Preparation plan (T-B), authorized steps only:** branch `feat/d19-m2cr-r4-diagnostic-execution` off the PR #19 merge; this docs-first commit (D); T-B regeneration in a fresh detached worktree at D by the established recipe — regenerate the four cascade artifacts (importable manifest, environment-freeze, infrastructure, protocol manifests) plus the v1.19 §4.5.1 interpreter-pin re-attestation, STOP on any delta beyond the informational worktree-header path, its manifest/pin cascade, and truthful report-currency changes; freeze commit E (the execution commit); the canonical closed-world launch packet at `docs/m2c_freeze/m2cr_r4_launch_packet_20260719_01.json` binding E, the six-member diagnostic chain, the ids below, the committed template pin `d62fee60…`, the detached execution-worktree identity, the derived evidence directory, and the frozen 8.0 h safety ceiling; exactly one prospective `authorization_granted` event `m2cr-ev-000002` (scope R4/diagnostic/`one_shot: true`, five-member `frozen_chain` at E, actual local grant date); commit packet + grant + factual status as G; fresh detached execution worktree at E; validations (launcher read-only mode, prospective ledger, `verify_chain(require_unconsumed=True)`, Layer-1a/1b pins, template and worktree identity, evidence-path absence, full hermetic suite at G); ONE bounded read-only Codex GPT-5.6-sol xHigh prelaunch audit of the exact E/G topology, packet, grant, command, A1 closure procedure, evidence handling, host-liveness plan, and consumption semantics, with every finding independently verified by Fable and no other reviewer or panel; no implementation modification; STOP on any material defect; return the final byte-exact one-shot launch authorization and STOP.
- **Fixed identities:** authorization `m2cr-auth-20260719-01`; launch attempt `m2cr-launch-20260719-01`; run id `diagnostic-m2cr-auth-20260719-01` (B3: kind plus authorization id); evidence directory `docs/m2c_evidence/diagnostic-m2cr-auth-20260719-01/`, used directly as `run_dir` and created only by a future authorized `--execute`; regeneration worktree `<session-scratchpad>/r4_regen_wt`; execution worktree `<session-scratchpad>/r4_exec_wt`.
- **NOT authorized by this entry:** starting caffeinate now; `launch_attempt_started`; `--execute`; spawning the production launcher or child; creating `docs/m2c_evidence/`; model/profile/diagnostic computation; closure events; R5 interpretation; any rerun.
- Startup note, recorded factually: at preparation start the local `origin/main` ref equaled the pinned merge SHA (GitHub-verified earlier the same session); an immediately preceding re-fetch attempt failed on DNS resolution (host offline), and preparation proceeds against the SHA the authorization itself pins.

**Alternatives considered:** a pre-execution closure-writer amendment (option A2 — rejected: it reopens implementation for machinery whose correctness authority is already the reviewed fail-closed acceptance stack, and D51 Update 1 records the reviewed design that closure events land in the post-run commit); `caffeinate -w <pid>` or a `pmset` change (rejected by the author for a simpler standalone `-i` process with an explicit recorded lifecycle); proceeding to launch without a prelaunch audit (never considered — the D45 failure class is exactly unreviewed execution-adjacent process).

**Status:** IN PROGRESS — preparation only. R4 execution remains NOT begun and NOT authorized; the one-shot launch requires a separate author authorization after the prelaunch audit. D45 remains permanently UNVALIDATED_ATTEMPT; `docs/m2c_freeze/gtoy_profile_result_v1.18.json` remains ABSENT.

### D52 — Update 1 (2026-07-19): T-B freeze E committed; canonical packet + prospective grant staged as G; no attempt, no execution

**Commits** (branch `feat/d19-m2cr-r4-diagnostic-execution` off PR #19 merge `66bba02`): D = `f745bcd` (docs-first D52 opening) · **E = `71ade35ef0ef6f424077f815070ff970bdd6c3d0`** (T-B freeze regen; the execution commit) · G = the packet+grant+this-update commit.

**T-B regeneration (fresh detached worktree at D, established recipe):** importable manifest 8,746,621 B / 39,969 lines, digest `7466a5f4172536e271e871c9a8e4d0cb44b35d1847a54d0ff3c617e807ecddb1` — the ONLY change is the informational header worktree path (`…/d9d6eac2…/scratchpad/r4_regen_wt`); all 39,968 entries proven byte-identical to the R3a body (diff over lines 2..39969 empty), zero added or removed. Environment-freeze manifest `33d9e9bc8f38dc62c9817b36812bd227a03b314ab74674336189d24ea78c1e49` (importable pin only); infrastructure manifest `8c2a59d9493f8e5a3cb43c46309067c8867b942237a0c8bf85bccc65e084435c` (code and r1_schemas sections byte-identical; only the environment-freeze and importable pins changed); protocol manifest `4835ce7f437177696969535d85bcf5ffaa3565150b121c312b40d13334a5b22b` (Layer-1a re-pin only; schema pin `076cdc2d…` and parameters pin `97625715…` unchanged). Interpreter pin re-attested per v1.19 §4.5.1 and byte-identical (`287a8461…`); child-env mapping, dependency lock, native-stack expectations, and preboundary attestation set untouched and byte-identical. Fixed-artifact total 8,870,676 B; report currency updated truthfully (F8 cross-check). Focused manifest/measure suites: 88 passed / 1 skipped (the pre-existing opt-in).

**Frozen E chain (six members):** `v117_canonical_sha256 = 65381bc774e894dd9aaf2207cadd9cfa2f2735dafceff4bb39492086a9e522e2` · `infrastructure_manifest_sha256 = 8c2a59d9…` · `environment_freeze_manifest_sha256 = 33d9e9bc…` · `protocol_manifest_sha256 = 4835ce7f…` · `execution_commit = 71ade35ef0ef6f424077f815070ff970bdd6c3d0` · `authorization_id = m2cr-auth-20260719-01`.

**Canonical launch packet:** `docs/m2c_freeze/m2cr_r4_launch_packet_20260719_01.json`, 1,675 canonical bytes (no trailing newline), sha256 `12032bbc317c3487fccd484f2aa88ded7d70e287d9cf4f8870e32d250d9a8b4c`; binds the 14-key closed world: E, the six-member chain, `m2cr-auth-20260719-01` / `m2cr-launch-20260719-01` / run id `diagnostic-m2cr-auth-20260719-01`, template pin `d62fee60…`, the three manifest pins at E, execution worktree `<session-scratchpad>/r4_exec_wt`, `evidence_dir docs/m2c_evidence/diagnostic-m2cr-auth-20260719-01/`, ceiling 8.0 h.

**Prospective grant appended (the ledger's second line, canonical + newline):** `authorization_granted`, `event_id m2cr-ev-000002`, `authorization_id m2cr-auth-20260719-01`, date 2026-07-19 (actual local grant date), scope {R4, diagnostic, `one_shot: true`}, five-member `frozen_chain` at E. Immediate validation via the committed authorities: `validate_ledger` ok (2 events, 0 errors); `verify_chain(record_kind="diagnostic", require_unconsumed=True)` all checks passed, including `grant_execution_commit` and `authorization_unconsumed`.

**Boundaries:** no `launch_attempt_started`, no `--execute`, no production launcher/child spawn, no `docs/m2c_evidence/`, no closure event, no model/profile/diagnostic computation, no caffeinate. Provenance of what DID run: the committed generators (importable-manifest walk, interpreter pin, environment-freeze, infrastructure, protocol manifests), sha256 hashing, the focused hermetic suites above, and the committed `validate_ledger`/`verify_chain` functions on the staged ledger. Pending after G: detached execution worktree at E; launcher read-only validation; full hermetic suite at G; ONE bounded read-only Codex gpt-5.6-sol xHigh prelaunch audit (Fable-verified); then STOP with the final one-shot launch authorization returned to the author.

### D52 — Update 2 (2026-07-19): step-9 gate failure at G diagnosed (pre-existing single-JSON ledger fixture); author Option-1 correction with refinements; fixture corrected as H; E/G superseded, never reused

**Gate failure at G, exact mechanism:** the full hermetic suite at G = `4689832` returned **1 failed / 1004 passed / 2 skipped**. The failure is `tests/test_m2cr_audit.py::test_consumed_deriving_from_historical_record_fails`, whose fixture parsed the ENTIRE committed ledger as one JSON document (`json.loads(LEDGER.read_text())`, an R1-era assumption true only while the ledger held exactly one line); with the grant `m2cr-ev-000002` as line 2 the fixture dies in setup — `json.decoder.JSONDecodeError: Extra data: line 2 column 1 (char 1558)`, char 1558 being the start of the grant line — and never reaches its assertion. Sweep: the ONLY such site in the suite (the only other committed-ledger reads are the path constant and `validate_ledger(LEDGER.read_text())`, which passes on the two-line ledger). The prepared state itself validated green throughout (launcher read-only 55/55 on packet `12032bbc…`; `validate_ledger` ok; `verify_chain(require_unconsumed=True)` all checks passed; evidence and v1.18 paths absent). Per the preparation authorization Fable modified nothing, STOPPED, and did NOT spend the one bounded Codex audit on the superseded topology.

**Author ruling (Option 1 with refinements, 2026-07-19):** (1) fix ONLY the brittle fixture — parse the committed ledger as JSONL and select exactly the D45 `historical_authorization_record` by `event == "historical_authorization_record"` AND `event_id == "m2cr-ev-000001"`, asserting the match is unique; no positional dependence; the semantic assertion (consumption deriving from a historical record is rejected) unchanged. (2) The test edit is an authorized expected importable-manifest delta; regenerate the complete T-B cascade from the corrected committed state; STOP on any delta beyond the test-file hash/entry from this exact correction, the regeneration worktree-header path, their mechanically derived pin cascade, and truthful report currency. (3) Preserve history: the correction and this update commit as H; no amend, reset, or force-push; E = `71ade35` and G = `4689832` remain in history and are never reused. (4) Create E2, then fresh identities `m2cr-auth-20260719-02` / `m2cr-launch-20260719-02` / `diagnostic-m2cr-auth-20260719-02`; append `m2cr-ev-000003` as the provenance `superseding_correction` targeting `m2cr-ev-000002`, then the new grant as `m2cr-ev-000004`. (5) Recorded fact, not to be overclaimed: `superseding_correction` validation only confirms its target exists — it does NOT operationally revoke or consume the old grant; therefore the obsolete `_01` packet is REMOVED from the G2 tree (`git rm`; preserved in history) so current HEAD contains no executable committed packet referencing the stale grant, the obsolete execution worktree at E is removed as housekeeping, and the `-01` grant is documented as unused, stale, and never attempted. (6) Explicit G2 verifications: `_01` packet absent from HEAD; no `-01` attempt or evidence; the new packet resolves only to the `-02` grant and E2; `validate_ledger` and `verify_chain(require_unconsumed=True)` pass; launcher read-only validation passes; focused regression passes; `python -m pytest -q` returns 1005 passed / 2 skipped / 0 failed; evidence path and the reserved v1.18 result remain absent. (7) Only after the topology is final, spend the ONE bounded Codex xHigh audit, its scope extended to the stale-grant/current-packet analysis; Fable independently verifies every finding. STOP conditions: if removing the old packet conflicts with a frozen rule, or if any current code still permits an accidental launch through the old grant at G2, STOP and report. No `--execute`, no caffeinate, no evidence, no diagnostic/scientific computation.

**H content:** exactly the authorized fixture correction in `tests/test_m2cr_audit.py` (JSONL parse; unique-match selection; uniqueness assertion) plus this update and SCRATCHPAD. Verified before commit: the corrected test PASSES against the current two-line ledger and the full audit file is 52 passed / 0 failed; a repo-wide sweep confirms the `_01` packet filename appears nowhere outside Notes (historical documentation, retained). Packet-removal conflict check: the `_01` packet is pinned by NO manifest, schema, or code path (it is an operational input; the infrastructure manifest pins 8 artifacts + 12 R2 code files + 2 R1 schemas, none of which is a packet), so `git rm` at G2 conflicts with no frozen rule.

### D52 — Update 3 (2026-07-19): E2 committed; packet-02 + provenance correction + fresh grant staged as G2; `_01` packet removed from the tree; no attempt, no execution

**Commits** (history preserved; nothing amended, reset, or force-pushed): D `f745bcd` · E `71ade35` (superseded, retained) · G `4689832` (superseded, retained) · H `473a3f8` (fixture correction + D52 Update 2) · **E2 = `baf5dd70b081cd020344c9d3c9433880a8faa116`** (T-B freeze at the corrected state; THE execution commit) · G2 = the packet+ledger+this-update commit.

**E2 regeneration (fresh detached worktree `r4_regen_wt2` at H, established recipe):** importable manifest 8,746,622 B / 39,969 lines, digest `31065760d9dd3ac67a4d4053a33e575ce587d2340b7397f807eb7f4d705c772f` — the delta vs the E version is EXACTLY the two authorized lines: the informational header worktree path (`…/r4_regen_wt2`) and the single `tests/test_m2cr_audit.py` entry (sha256 `5c805e4382c5300e1b5a1d1314112653796035ebc7ec56e671bd50a2bffc34cd`, size 37,521 B, byte-equal to the committed file at H); all other 39,967 entries byte-identical (diff line count 4 = two changed lines in old/new form). Environment-freeze manifest `b82909145e0e78541033d3e5ae59339d91ba0f35a7cedd8319a3a934d05a2e7f` (importable pin only); infrastructure manifest `222d8c162f03f6d259a32bd0555a5f369f551826042807117aae2965a4bd0947` (code and r1_schemas sections byte-identical; only the environment-freeze and importable pins changed); protocol manifest `f7b77c4f1a0db583fd59f345e7edfa70daa9c538ac5137480046e7b389ba6180` (Layer-1a re-pin only; schema pin `076cdc2d…` and parameters pin `97625715…` unchanged). Interpreter pin re-attested byte-identical (`287a8461…`); child-env mapping, dependency lock, native-stack expectations, preboundary set untouched. Fixed-artifact total 8,870,677 B; report currency updated truthfully. Focused suites at the regenerated state (manifest/measure/ceilings/protocol/audit): 140 passed / 1 skipped.

**Frozen E2 chain (six members):** `v117_canonical_sha256 = 65381bc774e894dd9aaf2207cadd9cfa2f2735dafceff4bb39492086a9e522e2` · `infrastructure_manifest_sha256 = 222d8c16…` · `environment_freeze_manifest_sha256 = b8290914…` · `protocol_manifest_sha256 = f7b77c4f…` · `execution_commit = baf5dd70b081cd020344c9d3c9433880a8faa116` · `authorization_id = m2cr-auth-20260719-02`.

**Canonical launch packet 02:** `docs/m2c_freeze/m2cr_r4_launch_packet_20260719_02.json`, 1,675 canonical bytes (no trailing newline), sha256 `451a02bb6a139cd0737306f1244df53c9117dbb574be4c9ea0617fa8a49d5dc4`; binds the 14-key closed world: E2, the six-member chain, `m2cr-auth-20260719-02` / `m2cr-launch-20260719-02` / run id `diagnostic-m2cr-auth-20260719-02`, template pin `d62fee60…`, the three manifest pins at E2, execution worktree `<session-scratchpad>/r4_exec_wt` (recreated fresh, detached at E2, tracked-clean; the obsolete E worktree removed per the refinement-5 housekeeping), `evidence_dir docs/m2c_evidence/diagnostic-m2cr-auth-20260719-02/`, ceiling 8.0 h. In the SAME commit the obsolete `_01` packet is `git rm`-ed, so current HEAD contains no executable committed packet referencing the stale grant.

**Ledger events appended (lines 3 and 4, canonical + newline each):** `m2cr-ev-000003` `superseding_correction` targeting `m2cr-ev-000002` — provenance-only per the author's ruling: it records the supersession and explicitly states it does NOT operationally revoke or consume the superseded grant; and `m2cr-ev-000004` `authorization_granted` (`m2cr-auth-20260719-02`, date 2026-07-19, scope {R4, diagnostic, `one_shot: true`}, five-member `frozen_chain` at E2). The `-01` grant is hereby documented as unused, stale, and never attempted: no `launch_attempt_started` cites it, nothing was consumed, and no evidence directory ever existed for it. Immediate validation via the committed authorities: `validate_ledger` ok (4 events, 0 errors); `verify_chain(record_kind="diagnostic", require_unconsumed=True)` for the `-02` chain: all checks passed, including `grant_execution_commit` and `authorization_unconsumed`.

**Boundaries:** unchanged — no `launch_attempt_started`, no `--execute`, no production launcher/child spawn, no `docs/m2c_evidence/`, no closure event, no diagnostic/scientific computation, no caffeinate. Pending after G2: the refinement-6 verification battery (packet-01 absence at HEAD, launcher read-only validation of packet-02, focused regression, full suite expecting 1005 passed / 2 skipped / 0 failed, evidence + v1.18 absence), then the ONE bounded Codex gpt-5.6-sol xHigh prelaunch audit (scope extended to the stale-grant/current-packet analysis), Fable-verified, then STOP with the final one-shot launch authorization.

### D52 — Update 4 (2026-07-19): G2 verification battery green; the ONE bounded Codex prelaunch audit returned REVISE; Fable verification confirms a closure-acceptance gap; STOP before any launch authorization

**Refinement-6 verification battery at G2 `3f5c68b` (all green):** `_01` packet absent from HEAD and from disk; zero `launch_attempt_started` events in the ledger (field-precise check; the single grep hit is the correction event's own prose); `docs/m2c_evidence/` and the reserved v1.18 instance absent; launcher read-only validation of packet-02 rc 0, 55/55 checks PASS; negative probe — the `_01` packet bytes restored as an untracked file at the old path fail launcher validation with 8 independent FAIL checks (`packet_committed`; three HEAD-manifest digests, which are the E2 versions; `worktree_head` at E2; three worktree manifest pins), probe file removed, tracked tree clean; focused regressions 140 passed / 1 skipped (manifest/measure/ceilings/protocol/audit) and 73 passed (r4_launch + launch-authority); full suite at G2: **1005 passed / 2 skipped / 0 failed**.

**ONE bounded Codex gpt-5.6-sol xHigh read-only prelaunch audit (scope included the stale-grant/current-packet analysis): VERDICT REVISE** — 1 BLOCKER, 2 MAJOR, 2 NOTE. Launch bytes and runtime safeguards assessed sound; objectives confirmed for topology, packet, grant/ledger, command, caffeinate isolation, and the stale-grant defenses; the A1 closure-acceptance stack objectives not confirmed.

**Fable independent verification of every finding (line-level, against source):**

- **F1 (BLOCKER) CONFIRMED — the A1 acceptance stack does not bind the closure BRANCH to captured marker presence.** A marker-present run mis-closed as `pre_payload_terminal_outcome` passes every named validator: `validate_ledger`'s pre-payload handling checks only ledger-internal state (`bistar_gp/m2cr/audit.py:298-319`); `verify_ledger_against_evidence` audits the marker only for ledger `payload_started` events and never requires marker ABSENCE on the pre-payload path (`audit.py:646-668`, shared terminal path `777-852`); `_ledger_authorization_state` flips consumed only on ledger events (`audit.py:871-902`), so `verify_chain(require_unconsumed=True)` would afterward report the scientifically consumed one-shot grant unconsumed. The terminal record's own `infra_fault.payload_started: true` (written by capture from the marker file, `capture.py:2684`) is cross-checked by nothing in the stack. Accidental mis-branching is in scope under the frozen v1.19 §4.5.13 threat model; **material**.
- **F2 (MAJOR) CONFIRMED as a code-level gap, adjudicated residual-class.** On the pre-payload branch the terminal record's `evidence.raw_manifest_sha256` binding is never verified against the actual raw-manifest file: the `_read_hashed_file(RAW_MANIFEST, event.raw_manifest_sha256)` and record-`evidence` comparisons are `terminal_outcome`-only (`audit.py:793-800`, `847-852`), and `_audit_raw_manifest` proves completeness and per-file digests but never the record's binding. Fable adjudication: capture derives that field from the file it just wrote, so the only path to the inconsistent state is post-capture mutation of the terminal record — the malicious same-user mutation class the frozen threat model records as an out-of-scope residual; no accidental path exists. Should nonetheless be closed by any future audit hardening.
- **F3 (MAJOR) CONFIRMED — the stack does not force a UNIQUE closure line set.** Admitted variants: non-consecutive (merely monotone) event ids; `authorization_consumed` before `terminal_outcome`; optional `historical_note` free text on every closure branch; optional and unvalidated `terminal_outcome.evidence_dir`. No wrong digest, status, or id VALUE passes (F1/F2 are the exceptions), but the SET is not unique. Closable without code by byte-exact prespecification of the closure lines in the final launch authorization plus an exact-set acceptance rule.
- **F4 (NOTE) CONFIRMED — audit-brief wording, not a defect.** The brief's objective-6 phrase "overflow yields `evidence_overflow` INFRA_FAILURE" overstated v1.20: frozen §4 replaces only certifiable candidates, elevates existing INFRA_FAILURE fault classes, and preserves ABORTED_BUDGET/NOT_STARTED statuses on breach; `capture.py:1019-1117` implements v1.20 exactly. Recorded as corrected.
- **F5 (NOTE) CONFIRMED and matches Update 3.** The `-01` grant remains formally prospective-and-unconsumed (`superseding_correction` validation checks only target existence, `audit.py:407-412`; corrections are ignored by authorization state, `audit.py:871-902`); no accidental G2 path reaches it (resurrected packet: 8 independent failures; hand-authored `-01` packet: commitment or E2 authority/worktree failures, and with E2 values the `grant_execution_commit` + five-member `grant_frozen_chain_equality` failures; checkout of superseded G: worktree-at-E2 failures); the residual requires deliberate author action on superseded history plus the author's own `--execute` act.

**Disposition: STOP (authorization step 11).** F1 is a material gap in the closure-acceptance stack and F3 compounds it; no implementation was modified and no launch authorization is returned. The launch-side preparation stands green and untouched: packet-02 `451a02bb…` at G2, prospective unconsumed grant `m2cr-ev-000004`, execution worktree at E2, every refinement-6 verification passed. The decision returns to the author: **(A)** strengthen ruling A1 without code — byte-exact prespecified closure line sets for both branches; a mandatory branch-equals-marker-presence acceptance check (marker file existence AND agreement of the terminal record's own `payload_started`/branch fields); a mandatory raw-manifest record-binding rehash on EVERY branch; an exact-set rule (consecutive ids from `next_event_id`, exemplar order, no `historical_note`, no `evidence_dir`) — closing F1–F3 operationally for this run while the committed stack is hardened after R4; or **(B)** an explicitly authorized pre-execution audit-hardening amendment (R3b-style: `audit.py` + tests + review + fresh freeze/grant cycle E3/G3 with `-03` identities) closing F1–F3 in committed code for R5/R6 as well.

### D52 — Update 5 (2026-07-19): author ruled OPTION B — narrow audit-hardening amendment; F1 treated as launch BLOCKER, F2 in-scope; hardening implemented (Codex) and Fable-verified; packet-02 removed from the tree (commit J)

**Author ruling (2026-07-19):** Option B — close F1–F3 in reviewed code before any R4 execution. F1 is a launch blocker; F2 is in scope (accidental pairing/corruption/incomplete capture can produce the mismatch without a malicious actor). History preserved (no amend/reset/force-push; D/E/G/H/E2/G2 retained and never reused). Implementation may be delegated to a fresh Codex gpt-5.6-sol xHigh agent with Fable independently verifying; the single reserved Fable adjudication is spent only on the final exact-head candidate. No `--execute`, caffeinate, launch attempt, evidence directory, or scientific/model/profile/diagnostic computation.

**Commit J (this commit) — the narrow implementation/test/docs amendment, exactly two source files + packet removal + docs:**

- `bistar_gp/m2cr/audit.py` — hardened `verify_ledger_against_evidence` (unchanged signature and return shape) plus a new deterministic closure API:
  - **F1 (BLOCKER) closed** — closure branch is now derived from CAPTURED EVIDENCE, never caller judgment. A new per-run-directory pass keys on the marker FILE (`payload_started.json`): marker present ⇒ the ledger MUST carry `payload_started` + `terminal_outcome` and MUST NOT carry `pre_payload_terminal_outcome` for that attempt; marker absent ⇒ the reverse. A helper `_record_asserts_payload_started` computes the terminal record's definite payload-start assertion (COMPLETED/ALGORITHM_STOP⇒True, NOT_STARTED⇒False, INFRA_FAILURE⇒`fault.payload_started`, ABORTED_BUDGET⇒none) and requires it to equal marker-file presence. A marker-present run mis-closed as pre-payload now fails on both the branch check and (defense in depth) the raw-manifest completeness audit, which lists the marker as a Layer-2 file so its deletion is caught.
  - **F2 (MAJOR) closed** — on BOTH terminal branches, the terminal record's `evidence.raw_manifest_sha256` is rehashed against the actual `RAW_MANIFEST.sha256` file and required equal (previously only the `terminal_outcome` branch checked it). Records with no evidence block (NOT_STARTED) skip the binding; `_audit_raw_manifest` completeness still runs.
  - **F3 (MAJOR) closed** — new `derive_closure_events(evidence_root, run_id, pre_closure_ledger_jsonl, *, date)` deterministically derives the unique ordered closure from captured evidence (marker file selects the branch; status copied from `terminal_record.json`; all digests recomputed from files; ids consecutive from the pre-closure ledger; no `historical_note`, no `evidence_dir`; every derived event schema-validated; the pre-closure ledger itself schema-validated and monotone-checked; the grant `frozen_chain`, marker chain, and record chain cross-checked member-by-member). `verify_closure(...)` requires the committed closure to byte-exactly equal the pre-closure ledger plus the canonical derived events, then additionally requires `validate_ledger` and the hardened `verify_ledger_against_evidence` to pass. A typed `ClosureDerivationError` fails closed on any inconsistency. The API is exported in `__all__`.
- `tests/test_m2cr_audit.py` — all existing tests preserved (the shared `_pre_payload_outcome` helper gains a backward-compatible defaulted `digest=SHA_A`); eight discriminating tests added proving rejection of: marker-present pre-payload closure; marker-absent payload closure; both terminal-record `payload_started` disagreements; pre-payload AND payload raw-manifest binding mismatches; and, via `verify_closure`, skipped/reordered/extra/duplicate-id/non-consecutive-id/injected-`historical_note`/injected-`evidence_dir` closure variants; plus a positive exact-sequence derivation check for both branches. File total 60 passed.
- `docs/m2c_freeze/m2cr_r4_launch_packet_20260719_02.json` — **`git rm`-ed in this commit** (author refinement 2). Grant `m2cr-ev-000004` (`m2cr-auth-20260719-02`) remains formally prospective and unconsumed but has NO launch packet at current HEAD while hardening is underway; it is superseded and never attempted, and its packet is preserved only in G2/history.

**Fable independent verification (line-level, of the exact working-tree diff before commit):** the two changed files are exactly `bistar_gp/m2cr/audit.py` and `tests/test_m2cr_audit.py` (no capture/schema/manifest/plan/prereg edit); `_record_asserts_payload_started` and the A1/A2/A3 hardening match the frozen consumption rule (plan §4.3, v1.19 §6); `derive_closure_events`/`verify_closure` are deterministic (date is a parameter; no `datetime.now`) and fail closed via `ClosureDerivationError`; the two pre-existing positive fixtures remain consistent with the tighter acceptance (both already bind `evidence.raw_manifest_sha256` to the file and agree marker↔branch↔assertion). Independent runs: full audit file 60 passed; audit-adjacent suites (r4_launch, launch_authority, terminal_records, records) 114 passed; module imports with the new API present. No implementation defect found; no correction pass needed at J. The fresh full-cycle Codex review and the single Fable adjudication follow at the final E3/G3 head (Update 6).

**Boundaries:** no ledger closure event was written (the API is committed, not exercised); no `--execute`, no attempt, no `docs/m2c_evidence/`, no caffeinate, no diagnostic computation. Pending: T-B regeneration from J → E3; packet-03 + provenance correction `m2cr-ev-000005` (targeting grant `m2cr-ev-000004`) + fresh grant `m2cr-ev-000006` (`m2cr-auth-20260719-03`, chain at E3) → G3; the refinement-9 verification battery; a fresh bounded Codex xHigh review of the final E3/G3 topology and closure machinery; the single Fable adjudication; then the final one-shot launch authorization. D45 stays permanently UNVALIDATED_ATTEMPT; the v1.18 instance path stays absent.

### D52 — Update 6 (2026-07-19): E3 committed; packet-03 + provenance correction + fresh grant staged as G3; only packet-03 launchable; no attempt, no execution

**Commits** (history preserved; nothing amended/reset/force-pushed): D `f745bcd` · E `71ade35` · G `4689832` · H `473a3f8` · E2 `baf5dd7` · G2 `3f5c68b` · Update-4 `af4bd37` · **J `b1ee5ecbda07ff71403ab7c466a8e94ab8f6ffde`** (audit-hardening + packet-02 removal) · **E3 `367667f77881469b47c901345015c72b70e2fc02`** (T-B freeze at the hardened state; THE execution commit) · G3 = the packet-03 + ledger + this-update commit.

**E3 regeneration (fresh detached worktree `r4_regen_wt3` at J, established recipe):** importable manifest 8,746,622 B / 39,969 lines, digest `c82ea053a6a6cf0ee13175c53d17640c0bb9df54fed9837563ffd5d8b78a2d46` — the delta vs the E2 version is EXACTLY the two authorized hardened source entries (`bistar_gp/m2cr/audit.py` sha `2adae520…` size 89,677; `tests/test_m2cr_audit.py` sha `ad0d1af5…` size 46,611) plus the informational header worktree path; all other 39,966 entries byte-identical; the file byte size is unchanged (both grown entries kept 5-digit sizes and fixed-width digests). Environment-freeze manifest `8b6795e58ce07c08be94ba9136edcf3f97431188eb57ac91e0a62b5aa76113bc` (importable pin only); infrastructure manifest `d8a3f302fc39b59a4210d65fefd10b67fce5a435245cd387bed952d120f7259d` (code section changed ONLY the `audit.py` pin; r1_schemas identical); protocol manifest `ace374a6d0b02eaf6321ac6b81d542ec532114235e166fac0190f499a0c179d2` (Layer-1a re-pin only; schema pin `076cdc2d…` and parameters pin `97625715…` unchanged). Interpreter pin re-attested byte-identical (`287a8461…`); child-env mapping, dependency lock, native-stack expectations, preboundary set untouched. Fixed-artifact total 8,870,677 B; report currency updated truthfully. Focused manifest/measure/protocol/ceilings suites: 88 passed / 1 skipped.

**Frozen E3 chain (six members):** `v117_canonical_sha256 = 65381bc774e894dd9aaf2207cadd9cfa2f2735dafceff4bb39492086a9e522e2` · `infrastructure_manifest_sha256 = d8a3f302…` · `environment_freeze_manifest_sha256 = 8b6795e5…` · `protocol_manifest_sha256 = ace374a6…` · `execution_commit = 367667f77881469b47c901345015c72b70e2fc02` · `authorization_id = m2cr-auth-20260719-03`.

**Canonical launch packet 03:** `docs/m2c_freeze/m2cr_r4_launch_packet_20260719_03.json`, 1,675 canonical bytes (no trailing newline), sha256 `54e28dcf04a511018d1fff70151500aed81e252eb43cc437df12076278215ef6`; binds the 14-key closed world: E3, the six-member chain, `m2cr-auth-20260719-03` / `m2cr-launch-20260719-03` / run id `diagnostic-m2cr-auth-20260719-03`, template pin `d62fee60…`, the three manifest pins at E3, execution worktree `<session-scratchpad>/r4_exec_wt` (recreated fresh, detached at E3, tracked-clean; the E2 worktree removed as housekeeping), `evidence_dir docs/m2c_evidence/diagnostic-m2cr-auth-20260719-03/`, ceiling 8.0 h. It is the ONLY launch packet present at HEAD (packet-01 removed at G2, packet-02 removed at J).

**Ledger events appended (lines 5 and 6, canonical + newline each):** `m2cr-ev-000005` `superseding_correction` targeting `m2cr-ev-000004` — provenance-only; explicitly does NOT operationally revoke or consume the superseded `-02` grant; and `m2cr-ev-000006` `authorization_granted` (`m2cr-auth-20260719-03`, date 2026-07-19, scope {R4, diagnostic, `one_shot: true`}, five-member `frozen_chain` at E3). The `-01` and `-02` grants are both documented unused, stale, and never attempted; neither has a launch packet at HEAD. Immediate validation via the committed authorities: `validate_ledger` ok (6 events, 0 errors); `verify_chain(record_kind="diagnostic", require_unconsumed=True)` for the `-03` chain: all checks passed, including `grant_execution_commit` and `authorization_unconsumed`.

**Boundaries:** unchanged — no `launch_attempt_started`, no `--execute`, no production launcher/child spawn, no `docs/m2c_evidence/`, no closure event, no diagnostic/scientific computation, no caffeinate. Pending: the refinement-9 verification battery (packet-02 absence, launcher read-only validation of packet-03, `validate_ledger`, `verify_chain`, full suite expecting all green with the 8 new tests, evidence + v1.18 absence, only packet-03 launchable), then the fresh bounded Codex gpt-5.6-sol xHigh review of the E3/G3 topology and closure machinery, then the single reserved Fable adjudication on the compact packet, then the final one-shot launch authorization.

### D52 — Update 7 (2026-07-19): refinement-9 battery GREEN; fresh Codex E3/G3 review APPROVE; single Fable adjudication NO MATERIAL DEFECT; Option-B amendment CLEARED; final one-shot launch authorization returned to author

**Refinement-9 verification battery at G3 `5cb3527` (all green):** packet-01 and packet-02 absent from HEAD and disk (only packet-03 present); `docs/m2c_evidence/` and the reserved v1.18 instance absent; ledger 6 events with zero `launch_attempt_started` (field-precise); launcher read-only validation of packet-03 rc 0, 55/55 checks PASS; the stale packet-02 negative probe (restored untracked) fails 8 independent checks (`packet_committed`; three HEAD-manifest disk digests = the E3 versions; `worktree_head` at E3; three worktree manifest pins), probe removed, tree clean; **full hermetic suite at G3: 1013 passed / 2 skipped / 0 failed** — the prior 1005 plus the 8 new discriminating audit tests; focused pre-commit runs: audit file 60 passed, adjacent (r4_launch/launch_authority/terminal_records/records) 114 passed, manifest/measure/protocol/ceilings 88 passed / 1 skipped.

**Fresh bounded Codex gpt-5.6-sol xHigh read-only review of the E3/G3 topology and closure machinery: VERDICT APPROVE** — zero BLOCKER/MAJOR/MINOR; three NOTEs, all classified non-defects; all nine objectives CONFIRMED (F1/F2/F3 closed; determinism; scope/topology; packet/chain/grant; stale-grant reachability; end-to-end chain consistency; no amendment regression). Fable independently verified each NOTE against source: (1) `NOT_STARTED` carries no `evidence.raw_manifest_sha256`, so the F2 binding has nothing to compare — a frozen-schema residual, not a regression (NOT_STARTED is non-consuming, still receives the raw-manifest completeness/per-file audit; F2's material case was the pre-payload INFRA_FAILURE, which is now bound); (2) `derive_closure_events` derives closure FIELDS, and `verify_closure`'s second stage (`verify_ledger_against_evidence` → `_audit_raw_manifest`) catches whole-bundle tampering, so no wrong closure is commit-eligible; (3) the stale `-01`/`-02` grants are formally unconsumed but packet-less, reachable only by deliberate history+worktree rollback plus explicit `--execute`. No confirmed conforming defect, so no correction pass was taken.

**Single reserved Fable adjudication (compact packet; the ONE Fable pass, Fable-token-conserving): VERDICT NO MATERIAL DEFECT.** All six questions NO-DEFECT, established via line-level source reading plus live read-only rejection probes: (1) closure-branch selection is keyed on the `payload_started.json` FILE per run directory and rejects a marker-present pre-payload closure on three independent errors and a marker-absent consuming closure symmetrically; (2) every definite terminal-record payload-start assertion disagreeing with marker presence errors in both the standing audit and derivation, and INFRA_FAILURE with a missing/non-boolean `fault.payload_started` fails closed; (3) the record-binding rehash fires on BOTH terminal branches when `evidence.raw_manifest_sha256` exists (probe reproduced the exact F2 error), the consuming branch adds a triple equality, `_audit_raw_manifest` enforces completeness+per-file on every branch, and the NOT_STARTED residual is confined and non-consuming; (4) `verify_closure`'s byte-exact equality against the canonical deterministic derivation rejects every reorder/duplicate/extra/extra-field/non-canonical variant, and `derive_closure_events` cannot emit a wrong schema-valid closure (digests recomputed from files, status copied from the schema-validated record, chains cross-checked, ids forced to max+1, only the format-constrained date free); (5) no accidental launch path through `-01`/`-02` at G3 (no citing packet exists; `packet_committed` + full `grant_frozen_chain_equality` + E3 manifest/worktree checks + explicit `--execute` each block it); (6) all five packet chain members byte-verify at HEAD, equal the `-03` grant `frozen_chain`, resolve to a detached-clean E3 worktree with matching pins, evidence absent, `-03` unconsumed with `require_unconsumed` enforced and one-shot flipping at `payload_started`. The one residual Fable named — coherent pre-audit destruction-plus-forgery of the whole run directory — is the frozen out-of-scope malicious mutation-and-restore class (v1.19 §4.5.13) and still cannot yield a relaunch (`launch_attempt_fresh`/`evidence_dir_absent`/`packet_committed` block reuse).

**Disposition:** the Option-B audit-hardening amendment is CLEARED. F1 (BLOCKER), F2 (MAJOR), and F3 (MAJOR) are closed in reviewed, committed code (commit J, execution commit E3); the post-run closure is now machine-derived (`derive_closure_events`) and independently gated (`verify_closure` + `validate_ledger` + `verify_ledger_against_evidence` + `verify_chain` + the frozen schemas + raw-manifest/per-file digests) with no manually invented closure JSON. Fable capacity is preserved for R5 (one adjudication spent this cycle). The final byte-exact one-shot R4 launch authorization is returned to the author (below). **HARD STOP before `--execute`, any launch attempt, `docs/m2c_evidence/`, caffeinate, closure events, and any model/profile/diagnostic computation.** At the eventual authorized launch the ledger's first appended event is `launch_attempt_started` `m2cr-ev-000007`; closure then derives from `m2cr-ev-000008`. D45 remains permanently UNVALIDATED_ATTEMPT; the v1.18 instance path stays absent.

### D52 — Update 8 (2026-07-19): R4 EXECUTED under the author's one-shot authorization; authorization CONSUMED; terminal status INFRA_FAILURE (attestation_fault); machine-derived closure BLOCKED by a pre-existing defect in `verify_ledger_against_evidence`; STOP with NO closure commit; routed to an authorized amendment

**This is a factual execution record. No decision-table application, no scientific-result interpretation (that is R5). The terminal status below is an infrastructure outcome, not a scientific STOP or result.**

**Pre-launch gate (all PASS):** tracked tree clean; packet-03 sha256 `54e28dcf…` byte-equal to HEAD; six-member E3 chain byte-verified (infra `d8a3f302…`, env `8b6795e5…`, protocol `ace374a6…`, v1.17 recomputed `65381bc7…e522e2`, commit E3, auth `m2cr-auth-20260719-03`); `validate_ledger` ok; `verify_chain(require_unconsumed=True)` all passed; launcher read-only validate 55/55; execution worktree detached-clean at E3; `docs/m2c_evidence/` and v1.18 absent.

**Host liveness (ruling B), recorded:** a standalone `/usr/bin/caffeinate -i` (absolute path `/usr/bin/caffeinate`), PID **25532**, started **2026-07-19T22:33:09Z**, terminated **2026-07-19T22:39:14Z** — started before the launcher, separate from it (reparented to PID 1), no `-w`/wrapper/`pmset`, killed after the launcher exited.

**Launch:** the committed launcher was invoked exactly once — `python -m bistar_gp.m2cr.r4_launch --packet docs/m2c_freeze/m2cr_r4_launch_packet_20260719_03.json --execute` — from the repo root, no wrapper/monkeypatch/instrumentation/extra flags. It appended `launch_attempt_started` `m2cr-ev-000007` (`m2cr-launch-20260719-03`) to the working-tree ledger, created `docs/m2c_evidence/diagnostic-m2cr-auth-20260719-03/`, and returned a durably published terminal record (report `ok: true` = launch MECHANISM succeeded; rc 0). All 55 launcher checks PASS.

**Factual terminal outcome:** `payload_started.json` **EXISTS** ⇒ the scientific authorization `m2cr-auth-20260719-03` is **CONSUMED** (permanent; the one-shot grant cannot be re-run). `terminal_status = INFRA_FAILURE`; `fault_class = attestation_fault`; `not_a_result = true`; `reconstructed = false`; `fault.payload_started = true`. Fault detail (verbatim, factual): the post-execution parent-side origin-binding authentication failed — `loaded module origin is unreadable and cannot be authenticated: _remote_module_non_scriptable -> …/r4_exec_wt/torch-git: [Errno 2] No such file or directory: '…/r4_exec_wt/torch-git'` (a torch-internal lazily-generated module whose `__file__` points at a non-existent `torch-git` path; §4.5.7/§4.5.10 origin/loader authentication). The diagnostic payload probed all **1,481** closure nodes (`nodes/` holds 1,481 records) and `payload.json` is present before the post-execution attestation faulted. **Digests:** evidence dir `docs/m2c_evidence/diagnostic-m2cr-auth-20260719-03/`; `terminal_record_sha256 = 0d17ddb811f51d559a7637bb6358ac83b2dc63d76a7600dc2432c9a1657b6336`; `raw_manifest_sha256 = 6c1a50b05f3e7529e64cd8c332ec2a11db09c11f3aafaee1aa155e8b2ab16508`; `payload_started_sha256 = 1fd32af13086bdfda2105dc7472087589096113f71738cbf0c54b55d4b11072c`; `payload_sha256 = 62bf08e279c3566514770dc0715f6ccc0c9d6e3baa3721884d3a169d1b2fc201`.

**Closure BLOCKED — the mandated acceptance stack fails on a pre-existing defect in `verify_ledger_against_evidence` (NOT introduced by the Option-B hardening; `git diff af4bd37 b1ee5ec -- bistar_gp/m2cr/audit.py` shows the attestation-resolution line untouched).** `derive_closure_events` correctly produced the three payload-branch events (`payload_started` `m2cr-ev-000008`, `terminal_outcome` INFRA_FAILURE `m2cr-ev-000009`, `authorization_consumed` `m2cr-ev-000010`), but `verify_closure`/`verify_ledger_against_evidence` reject with `dangling digest; evidence file is missing` for four marker attestations. Root cause: `verify_ledger_against_evidence` resolves each marker attestation NAME to `<name>.json`, but `capture.py`'s frozen `_MANDATORY_MARKER_ATTESTATIONS` maps 4 of the 10 mandatory marker-names to DIFFERENT on-disk file stems — `path_and_stage_a`→`stage_a`, `bytecode_scan`→`bytecode`, `sourceless_check`→`sourceless`, `importable_manifest_pre`→`manifest_pre`. The auditor looks for `bytecode_scan.json`/`path_and_stage_a.json`/`sourceless_check.json`/`importable_manifest_pre.json`, which capture never writes, so it rejects legitimate real evidence. The defect was latent because every hermetic fixture used a single self-named attestation (`environment` → `environment.json`); the FIRST real payload-branch closure audit surfaced it. Neither the two Codex reviews nor the Fable adjudication caught it — none exercised real multi-attestation capture output; the fail-closed acceptance stack caught it by refusing to certify, exactly as designed.

**Action taken (per ruling A1 "any disagreement means STOP with no closure commit" and the R4 contract "if the post-run closure machinery is defective, STOP and route to an explicitly authorized pre-execution amendment; never improvise a ledger-closing script"):** **STOPPED. No closure was committed. No fix was improvised. Nothing was committed for the closure, the ledger change, or the evidence directory.** The working tree PRESERVES, uncommitted: the ledger with `launch_attempt_started` `m2cr-ev-000007` (committed HEAD ledger stays at 6 events) and the complete evidence directory (untracked). Only this factual D52/SCRATCHPAD record is committed.

**Chain-consistency consideration for the amendment (author decision required):** the defect is a cross-module naming disagreement between two chain-pinned R2 modules (`verify_ledger_against_evidence` in `audit.py` vs `_MANDATORY_MARKER_ATTESTATIONS` in `capture.py`). Fixing it edits a chain-pinned module, which changes the infrastructure manifest and thus supersedes the E3 freeze. IMPORTANT: the CLOSURE itself binds the terminal record's chain to the GRANT's `frozen_chain` (both at E3, self-consistent) and does NOT depend on the current `audit.py` hash — so a fixed auditor can verify the ALREADY-CAPTURED E3 evidence against the E3 grant WITHOUT re-running the diagnostic (the consumed `-03` grant forbids a re-run regardless). The author must decide the amendment shape: e.g., (a) an authorized audit-only fix to `verify_ledger_against_evidence`'s attestation-name→file resolution (mirroring `_MANDATORY_MARKER_ATTESTATIONS`) plus a real-evidence regression test, applied on a new commit, then re-derive and commit the `-03` closure over the preserved evidence, accepting that E3's freeze is superseded by the fix commit while the closure remains bound to the E3 grant/record; versus (b) any alternative the author prefers. This is explicitly OUT of R4 execution scope and returned to the author.

**Boundaries:** no decision-table application, no scientific-result claim, no R5, no rerun, no amendment performed. The `-03` grant is CONSUMED and its attempt is recorded; D45 remains permanently UNVALIDATED_ATTEMPT; the v1.18 instance path stays absent. **HARD STOP pending the author's amendment authorization.**

### D52 — Update 9 (2026-07-19): author authorized a narrow post-execution closure-auditor amendment; Step-1 pre-amendment immutability baseline recorded; import-safety checks pass; manifest/pinning consequence surfaced before editing any pinned module

**Author authorization (2026-07-19):** a narrow post-execution R4 closure-auditor amendment ONLY. The consumed `-03` attempt is never rerun; the captured E3 evidence and the uncommitted `launch_attempt_started` line are preserved byte-for-byte (no modify/delete/regenerate/normalize/reconstruct of any evidence file); no `payload.json` interpretation, no decision table, no origin-binding repair, no R5, no v1.18. Fix the attestation logical-name → evidence-file mapping at one authoritative source (prefer promoting capture's existing mapping to a public immutable contract the auditor consumes; no duplicated mapping); confine the amendment to the mapping contract + `verify_ledger_against_evidence` + discriminating tests + factual provenance; add a hermetic fixture with all ten real marker attestations incl. the four aliased names; Codex may implement (Fable reserved for R5); commit implementation/tests separately EXCLUDING the ledger and `docs/m2c_evidence/`; do NOT regenerate T-B or create a new grant; then run the fixed auditor read-only over the preserved evidence, re-hash for byte identity, re-derive closure 000008–000010, and commit the ledger closure + evidence only if the full acceptance stack passes; else STOP.

**Step 1 — pre-amendment immutability baseline (recorded factually; the evidence and ledger were NOT staged and will NOT be staged in any implementation commit):**
- Evidence directory `docs/m2c_evidence/diagnostic-m2cr-auth-20260719-03/`: **1503 files** (1481 `nodes/` + 22 root). **EVIDENCE_TREE_DIGEST** (sha256 over the sorted `sha256␠␠relpath` inventory of every regular file) = `7154212a5256b74f9feab99bfdd4c06c9c0fa2fbefa4d391d817c517b3fb4e9e`. Full per-file inventory preserved off-tree for the post-amendment byte-identity re-hash.
- Working-tree ledger `docs/m2c_freeze/m2c_authorization_ledger.jsonl` (with the uncommitted `launch_attempt_started m2cr-ev-000007`): **LEDGER_BYTES_DIGEST** = `7ea5114b7b2c768da21fbdf98e26727240be1b26e0ba42a35fe8935c5d6c88df` (5686 bytes, 7 lines). Committed HEAD ledger remains 6 events.
- Key evidence digests (unchanged from Update 8): `terminal_record_sha256 = 0d17ddb8…`; `raw_manifest_sha256 = 6c1a50b0…`; `payload_started_sha256 = 1fd32af1…`; `payload_sha256 = 62bf08e2…`.

**Step 2 — import-safety checks (PASS):** `capture.py` does not import `audit.py` (no cycle if the auditor consumes capture's mapping); the child bootstrap imports neither; `diagnostic_payload` already imports `capture` at payload time, so capture is already an authenticated child import; `audit.py` already imports capture lazily (`enumerate_bootstrap_closure`). A parent-side `audit → capture` edge for the mapping is cycle-free and adds nothing to the child's pre-scientific import path. No new shared module is required.

**Manifest/pinning consequence — surfaced before editing any pinned module (author decision requested):** the auditor fix necessarily edits `bistar_gp/m2cr/audit.py` (it holds `verify_ledger_against_evidence`), and the preferred public-contract approach also edits `bistar_gp/m2cr/capture.py`. Both are chain-pinned R2 modules whose sha256 the E3 infrastructure manifest (code section) and the E3 importable-artifact manifest (worktree entries) pin. Two committed-tree consistency tests enforce those pins against the actual code — `tests/test_m2cr_infrastructure_manifest.py::test_committed_infrastructure_manifest_matches_tree` (runs `verify_infrastructure_manifest(COMMITTED_MANIFEST)`) and `::test_committed_importable_manifest_worktree_entries_match_tree` — so editing either module WITHOUT regenerating T-B makes both tests fail. Both provide the documented `M2CR_ALLOW_MISSING_COMMITTED_MANIFEST=1` "orchestrator regeneration window" skip. Regenerating T-B is forbidden by the authorization (step 6) and would move the current `infrastructure_manifest_sha256` off the `d8a3f302…` value the `-03` grant/record cite. **HARD STOP: awaiting the author's decision on (a) handling those two consistency tests via the documented skip while the committed freeze intentionally lags the amended auditor, versus regenerating T-B; and (b) editing `capture.py` for the public contract versus an audit-only consumption of capture's existing single-source mapping. No pinned module edited yet.**

### D52 — Update 10 (2026-07-19): author chose (a) Option B [regenerate for a self-consistent tree] + (b) Option 1 [public immutable contract]; closure-auditor amendment implemented (commit J = the auditor fix); E3 held immutable; packet-03 removed from HEAD

**Author decision (2026-07-19):** (a) **Option B** — regenerate the current manifests so the final tree is self-consistent and ORDINARY unflagged `python -m pytest -q` is fully green; the `M2CR_ALLOW_MISSING_COMMITTED_MANIFEST=1` flag is used ONLY inside the bounded regeneration window, never left as a permanent requirement. (b) **Option 1** — promote capture's existing mapping to a public immutable contract the auditor consumes (single source, no duplication; values byte-for-byte unchanged; an ownership/export correction, not a capture behaviour change). Plus: E3 remains the immutable execution freeze (do not amend/reinterpret E3, its grant, terminal record, or evidence; never describe the regenerated manifests as the chain under which `-03` ran); preserve the committed Step-1 immutability baseline; keep the uncommitted launch-attempt line and all 1503 evidence files untouched and excluded from implementation/regeneration commits; commit the mapping/auditor/tests amendment first; remove packet-03 from the tree (preserve its bytes/hash in history, cite it); regenerate T-B from a fresh detached worktree with only the permitted deltas; prove E3 retrievability + closure chain-separation + unflagged pytest before running the fixed auditor on preserved evidence; one fresh read-only Codex xHigh review (Fable reserved for R5); certify closure only if the complete stack passes.

**Import-safety re-confirmed (author requirement):** `capture.py` does not import `audit.py`, and none of capture's top-level deps (`events`, `payload_boundary`, `records`, `serialization`) import `audit` — so a parent-side `audit → capture` edge is cycle-free (verified empirically: `import capture; import audit` succeeds). The child bootstrap imports neither module; `audit.py` is never in the child's pre-scientific import path (only `capture` is, via `diagnostic_payload`, already authenticated), so consuming the mapping introduces **no import cycle and no pre-scientific import-path change**.

**The amendment (this is the auditor-fix code amendment — the intentional edits, exactly three source files):**
- `bistar_gp/m2cr/capture.py`: promoted the private `_MANDATORY_MARKER_ATTESTATIONS` to a public immutable `MANDATORY_MARKER_ATTESTATIONS = MappingProxyType({…})` with the ten byte-identical name→stem entries (four aliased: `path_and_stage_a`→`stage_a`, `bytecode_scan`→`bytecode`, `sourceless_check`→`sourceless`, `importable_manifest_pre`→`manifest_pre`); kept `_MANDATORY_MARKER_ATTESTATIONS = MANDATORY_MARKER_ATTESTATIONS` as an internal alias so every existing capture reference is byte-unchanged; added the public name to `__all__`. No capture runtime behaviour changes.
- `bistar_gp/m2cr/audit.py`: top-level `from bistar_gp.m2cr.capture import MANDATORY_MARKER_ATTESTATIONS`; in `verify_ledger_against_evidence`'s attestation loop, resolve `stem = MANDATORY_MARKER_ATTESTATIONS.get(name, name)` and read `attempt / f"{stem}.json"` (was `f"{name}.json"`). Single authoritative source; no duplicated mapping.
- `tests/test_m2cr_audit.py`: added a hermetic real-layout fixture writing all ten mandatory attestations (marker records the logical names; four resolve to aliased stems) and five discriminating tests — the ten-attestation positive; aliased-file-missing, aliased-under-old-buggy-name, and aliased-digest-mismatch negatives (each fails closed); and a single-authority assertion (`audit.MANDATORY_MARKER_ATTESTATIONS is capture.MANDATORY_MARKER_ATTESTATIONS`). The fixture does not depend on the untracked scientific evidence directory.

**Verification of the fix (read-only, before commit):** the full acceptance stack now PASSES on the REAL preserved `-03` evidence — `derive_closure_events` yields the payload-branch events `m2cr-ev-000008` (payload_started) / `000009` (terminal_outcome INFRA_FAILURE) / `000010` (authorization_consumed), and `verify_closure`, `verify_ledger_against_evidence`, `validate_ledger`, and `verify_chain` all return ok. Focused suites: `tests/test_m2cr_audit.py` 64 passed (the single remaining failure is the mid-flight dangling-ledger stream test, which the closure resolves — NOT a regression); capture/launch-authority/r4_launch/terminal_records/payload_boundary 211 passed.

**Packet-03 removed from HEAD (author constraint 4):** `docs/m2c_freeze/m2cr_r4_launch_packet_20260719_03.json` is `git rm`-ed in the amendment commit. Its exact bytes are preserved in history at **G3 `5cb3527`** — git blob `f7fa1a02eb788fc168416f331ee8eba3cab6944b`, sha256 `54e28dcf04a511018d1fff70151500aed81e252eb43cc437df12076278215ef6` (retrievable byte-exact via `git show 5cb3527:docs/m2c_freeze/m2cr_r4_launch_packet_20260719_03.json`). Current HEAD contains NO executable `-01`, `-02`, or `-03` packet. Packet-03 was already exercised and its grant `m2cr-auth-20260719-03` is consumed; no packet is needed to close the run.

**E3 immutability (author constraint 1), recorded separately:** the **execution freeze E3 = `367667f77881469b47c901345015c72b70e2fc02`** with its original chain `v117 65381bc7…e522e2` / infra `d8a3f302…` / env `8b6795e5…` / protocol `ace374a6…` is the immutable chain under which `-03` executed and is cited by the `-03` grant, terminal record, and marker — it is NOT amended, reinterpreted, or replaced. The **post-execution auditor amendment** (this commit and its T-B regeneration, digests recorded in the next update) produces the CURRENT-tree freeze for self-consistency; the regenerated manifests are the current-tree state, NOT the chain under which `-03` ran. Closure verification authenticates the historical E3 chain: the four freeze artifacts (v1.17 source, infrastructure/environment-freeze/protocol manifests) are retrievable byte-exact from commit E3, and their hashes equal the `-03` grant's `frozen_chain` and the terminal record's chain — the grant itself lives in the ledger at G3 and later (not in E3's tree, whose ledger ends at the `-02` grant), and the terminal record is preserved evidence; closure never substitutes the regenerated live L manifests. (D52 Update 11 wording precision, per the fresh Codex review's NOTE 2.)

**Boundaries:** the ledger and `docs/m2c_evidence/` are NOT staged in this amendment commit; the Step-1 baseline (`EVIDENCE_TREE_DIGEST 7154212a…`, `LEDGER_BYTES_DIGEST 7ea5114b…`) is unchanged. No new grant/packet/execution-worktree/`--execute`/caffeinate/diagnostic/Mauna/v1.18. Pending: T-B regeneration → self-consistent tree; E3-retrievability + chain-separation + unflagged-pytest proofs; one fresh Codex xHigh review; then the fixed auditor over the preserved evidence with a 1503-file byte-identity re-hash and the machine-derived closure commit. D45 stays UNVALIDATED_ATTEMPT; the `-03` terminal outcome remains `INFRA_FAILURE / attestation_fault / not_a_result: true`.

**Commits: K = `2797cef0a738a1d6efada6881aef31b5303a5b79`** (the amendment: capture + audit + tests + packet-03 removal + D52 Update 10; ledger/evidence excluded). **L = the T-B regeneration at K** (fresh detached worktree `r4_regen_wt4`): importable manifest `7ed82075aa5074843fddedc78893eecbd097564aef9db48d812f808209afcc5e` (8,746,622 B; body delta EXACTLY the three amended entries — `audit.py` 89,677→90,343, `capture.py` 151,906→152,853, `tests/test_m2cr_audit.py` 46,611→52,560 — plus the header path; all other 39,965 entries byte-identical); environment-freeze `41e983c0ce0e29db145cd1b8da6ea339a519d717b6f5c4d22c3e7a5e68b1288e`; infrastructure `8565350604719f92c5e75227cd1109658365e6d71d81d38dcb12ac200f27dd47` (code section changed EXACTLY the `audit.py` + `capture.py` pins; r1_schemas identical); protocol `8cd1e98b03cb99e3df970d6f86449d60c3edb2a03a79160a003f275d8e1fd704` (Layer-1a re-pin only; schema/parameters pins unchanged); interpreter pin re-attested byte-identical `287a8461…`; child-env mapping, dependency lock, native-stack expectations, preboundary set byte-identical; fixed-artifact total 8,870,677 B; report currency updated truthfully. **These are the CURRENT-tree self-consistency freeze digests, explicitly NOT the chain under which `-03` ran (that is the immutable E3 `d8a3f302…`).** The `M2CR_ALLOW_MISSING_COMMITTED_MANIFEST=1` flag was NOT used; ordinary manifest-consistency verifiers pass at L.

### D52 — Update 11 (2026-07-19): constraint-6 proofs green; fresh Codex xHigh review APPROVE (2 wording NOTEs, corrected); Fable NOT invoked (reserved for R5); one correction pass → K2/L2; closure pending

**Constraint-6 proofs (read-only, all PASS):** (a) every E3 freeze artifact is retrievable byte-exact from commit E3 and its hash equals the `-03` grant `frozen_chain` and the terminal record chain (infra `d8a3f302…`, env `8b6795e5…`, protocol `ace374a6…`, v117 `65381bc7…`, commit E3); (b) chain separation — the current-tree infra manifest (`8565350…`, L) DIFFERS from the `-03` chain (`d8a3f302…`, E3); closure authenticates the historical E3 value, not the current manifest; (c) `verify_chain` for the preserved `-03` record passes against its E3 grant. Constraint-8 byte-identity (read-only): all 1503 evidence files re-hash to `EVIDENCE_TREE_DIGEST 7154212a…` and the working-tree ledger to `LEDGER_BYTES_DIGEST 7ea5114b…` — untouched through K/L.

**Fresh bounded Codex gpt-5.6-sol xHigh read-only review of the exact amendment + chain-separation logic (HEAD L `7a6871d`): VERDICT APPROVE** — zero BLOCKER/MAJOR/MINOR; all seven objectives CONFIRMED (fix correctness: the auditor resolves all ten names, four aliased + six self-named, and fails closed on missing/renamed/tampered aliased files; single authority, no duplication; capture behaviour-neutral; import-safe; scope limited to the three source files + packet-03 removal + the four regenerated artifacts + report currency, r1_schemas identical; E3 immutable with proven chain separation; no new runtime defect). Two NOTEs, both documentation/provenance WORDING (non-operational), independently verified by Fable-orchestrator against source and CORRECTED in a single conforming correction pass: (1) `verify_ledger_against_evidence`'s docstring still said name→`NAME.json`; corrected to describe the `<STEM>.json` mapping resolution with the four aliases; (2) this entry's Update-10 wording implied the `-03` grant/record are retrievable from commit E3 — corrected above to state the freeze ARTIFACTS are retrievable from E3 and their hashes match the grant (in the G3+ ledger) and the record (preserved evidence). **Per the author's instruction, Fable was NOT invoked; the single reserved Fable adjudication remains preserved for R5.**

**Correction pass — K2 = the docstring + D52-wording corrections** (`bistar_gp/m2cr/audit.py` docstring + this Notes text; ledger/evidence excluded); **L2 = the T-B re-regeneration at K2** (audit.py's byte change re-pins the importable/environment/infrastructure manifests + protocol Layer-1a; capture.py unchanged since K, so its pin is unchanged from L; digests recorded in the commit). E3 stays immutable; the closure's E3 binding is unaffected. After L2: unflagged `python -m pytest -q` green with the closure written, then the fixed auditor over the preserved evidence, the 1503-file byte-identity re-hash, and the machine-derived closure + evidence commit.

### D52 — Update 12 (2026-07-19): R4 CLOSURE CERTIFIED — machine-derived `-03` closure committed over the preserved E3 evidence, certified by the post-execution auditor amendment; execution occurred under E3; terminal outcome INFRA_FAILURE / attestation_fault / not_a_result: true

**Amendment commit chain (all preserved):** K `2797cef` (auditor fix + packet-03 removal) → L `7a6871d` (regen) → K2 `6636929` (Codex-NOTE wording corrections) → L2 `76f3c39` (re-regen). L2 manifest-consistency tests pass UNFLAGGED (`M2CR_ALLOW_MISSING_COMMITTED_MANIFEST` never used at the committed state); capture-heavy suites 211 passed; audit file all green with the closure written.

**Closure derivation and certification (author steps 8–9):** with the amended auditor at L2, `derive_closure_events` over the preserved evidence `docs/m2c_evidence/diagnostic-m2cr-auth-20260719-03/` produced exactly the payload-branch closure — `m2cr-ev-000008` `payload_started`, `m2cr-ev-000009` `terminal_outcome` (record_kind diagnostic, status **INFRA_FAILURE**), `m2cr-ev-000010` `authorization_consumed` (derived from 000008) — appended after the preserved `launch_attempt_started` `m2cr-ev-000007` (ledger lines 1–7 byte-identical; the attempt line untouched). The COMPLETE acceptance stack passed: `derive_closure_events`, `verify_closure` (byte-exact deterministic closure), `validate_ledger` (10 events), `verify_ledger_against_evidence` (every marker attestation — including the four aliased — resolved and digest-verified; raw-manifest completeness + per-file digests), `verify_chain` (the `-03` record chain against its E3 grant), and both frozen schemas. **Certification provenance, stated precisely: execution occurred under the immutable freeze E3 `367667f` (chain `d8a3f302…`), and closure CERTIFICATION used the reviewed post-execution auditor amendment (commits K/K2 at L2 `76f3c39`); the two are distinct and the terminal record/grant remain E3-bound.**

**Byte-identity (author steps 2, 8):** all **1503** evidence files re-hash to the pre-amendment `EVIDENCE_TREE_DIGEST 7154212a…` (byte-identical throughout the amendment); the pre-closure ledger's first seven lines are byte-identical to the Step-1 `LEDGER_BYTES_DIGEST 7ea5114b…` baseline. No evidence file was modified, deleted, regenerated, normalized, or reconstructed; `payload.json` was not interpreted; the decision table was not applied; the origin-binding INFRA_FAILURE was not repaired.

**Factual terminal outcome (unchanged, not interpreted):** `INFRA_FAILURE` / `fault_class: attestation_fault` / `not_a_result: true`; authorization `m2cr-auth-20260719-03` CONSUMED (marker present); 1,481 nodes probed; `payload.json` present. Digests: `terminal_record_sha256 0d17ddb8…`; `raw_manifest_sha256 6c1a50b0…`; `payload_started_sha256 1fd32af1…`; `payload_sha256 62bf08e2…`.

**Commit of record:** the closure commit stages the complete evidence directory `docs/m2c_evidence/diagnostic-m2cr-auth-20260719-03/` (1503 files) and the 10-event ledger; a Draft evidence PR is opened. **HARD STOP before Ready, merge, R5, interpretation, origin-binding repair, or any rerun.** No new grant/packet/execution-worktree/`--execute`/caffeinate/diagnostic/Mauna/v1.18. D45 remains permanently UNVALIDATED_ATTEMPT; the reserved v1.18 result path stays absent. R5 alone audits the committed record and applies §6.3 rows 1–10 (a diagnostic terminal status other than COMPLETED routes to PRESERVE_STOP by row 2 — but that determination is R5's, not this session's).

## D53: M2cR milestone R5 — independent read-only record audit of the R4 `-03` closure (Draft PR #20 @ `f78d16a`) returned APPROVE_RECORD; §6.3 decision table applied mechanically, first match row 2 (PRESERVE_STOP; no amendment); author confirmed; Notes-only closeout — 2026-07-19

**Milestone.** R5 (plan §8) is the independent record audit and mechanical §6.3 rule application over the committed R4 diagnostic record. It is adjudication only: it authorizes no execution, no amendment, and no interpretation of `payload.json`'s scientific contents (the terminal record carries `not_a_result: true`).

**Audit provenance (stated precisely).** The independent read-only R5 audit was performed by **Fable Max** at exact evidence head **`f78d16a787962a37d35dd9abb9d309f47798ae86`** (Draft PR #20 head; base `main`; `isDraft: true`). It is an **AI audit, not a submitted GitHub review or CI result.** Fable returned **`APPROVE_RECORD`**. Three provenances stay distinct: execution occurred under the immutable freeze **E3 `367667f`** (chain `d8a3f302…`); closure **certification** used the reviewed post-execution auditor amendment at **L2 `76f3c39`** (D52 Updates 10–12); the **R5 audit** is Fable Max at `f78d16a`. This D53 Notes-only closeout was authored under **Opus 4.8**, and its final head was **not itself re-audited by Fable**.

**Independently confirmed (read-only, recomputed from bytes / git objects — not adopted from D52).**
- **Record auditability:** the committed `verify_ledger_against_evidence` over `docs/m2c_evidence/` returns ok=True, 10 events, 0 errors, 16/16 digest checks passed. `derive_closure_events` re-derives `m2cr-ev-000008/000009/000010` **byte-identical** to committed ledger lines 8–10; ledger lines 1–7 byte-identical to the Step-1 `LEDGER_BYTES_DIGEST 7ea5114b…`.
- **Evidence integrity + immutability:** 1503 tracked evidence files; `RAW_MANIFEST.sha256` (1501 entries) all rehash OK and is two-way complete over the directory; recomputed `EVIDENCE_TREE_DIGEST = 7154212a…` equals the pre-amendment baseline (evidence unaltered by the closure-auditor amendment). Cited digests reproduce: terminal `0d17ddb8…`, raw-manifest `6c1a50b0…`, marker `1fd32af1…`, payload `62bf08e2…`.
- **Effective E3 chain retrieved from E3 (not silently replaced by regenerated manifests):** env `8b6795e5…`, infra `d8a3f302…`, protocol `ace374a6…`, v1.17 canonical `65381bc7…` all reproduce from commit E3 and equal the `-03` grant `frozen_chain`, the marker chain, and the terminal-record chain; the current-tree regenerated manifests hash differently (env `69028f69…`, infra `4bf7bec0…`, protocol `a268dd85…`), confirming closure authenticated the historical values. The R3 schemas and frozen protocol parameters are byte-identical E3↔HEAD.
- **payload.json** validates cleanly against the frozen `m2c_diagnostic_record.schema_v1.json` (Draft 2020-12, local committed resolution) — schema-complete; scientific contents not interpreted.
- **Absences:** `gtoy_profile_result_v1.18.json` absent; no launch packet tracked at HEAD; the `-03` grant CONSUMED (ev-000010 derives from ev-000008; marker present).

**Mechanical §6.3 application (committed `evaluate_decision_table`, [diagnostic.py:266](../bistar_gp/m2cr/diagnostic.py)).** Inputs: `instance` = committed `payload.json`; `terminal_status` = `INFRA_FAILURE` (from the authoritative Layer-4 terminal record, not the payload's internal `status: COMPLETED`); `evidence_complete` = True. **Row 1 does not match** — the schema-valid instance records `purity.pass: true` (verified, not assumed). **Row 2 is the first match** — `INFRA_FAILURE ≠ COMPLETED` fires the terminal-status disjunct alone (result identical under either `evidence_complete` value). Exact evaluator output: **`{"row": 2, "track": "evidence_incomplete_no_amendment"}`** → frozen disposition **PRESERVE_STOP; evidence incomplete; no amendment**. Per first-match discipline, no later row was adjudicated.

**Two NOTE-level observations (accurate; no action; no historical commit or evidence rewritten).**
1. The closure commit message says "ev 000008-000010" while its ledger diff appends **000007–000010**; not a substantive discrepancy — ev-000007 (`launch_attempt_started`) was written by the launcher at launch time and preserved byte-identical, and the machine-derived subset is exactly 000008–000010 (D52 Update 12 states this precisely).
2. D52 Update 9's `EVIDENCE_TREE_DIGEST` recipe is underspecified on sort key and terminator; it reproduces `7154212a…` exactly only under sort-by-relpath of `<sha256>␠␠<relpath>` lines with a trailing newline.

**Disposition + author confirmation.** The record is independently auditable and valid. Diagnostic terminal disposition: `INFRA_FAILURE / attestation_fault / not_a_result: true` → §6.3 **row 2 → PRESERVE_STOP** (`evidence_incomplete_no_amendment`). **Row 8 did not fire**, so no B4 amendment exists; **R6 is unreachable** (plan §8: any non-row-8 R5 outcome ends the arc with the STOP preserved); this outcome authorizes **no origin-binding repair, no rerun, no scientific-result interpretation**. The author confirmed the R5 mechanical application in their own words (2026-07-19) and adopted PRESERVE_STOP; no amendment.

**R5 closeout (this entry).** Notes-only: D53 + SCRATCHPAD/CHATLOG. No code, schema, manifest, ledger event, evidence file, freeze artifact, payload, or v1.18 path modified. PR #20 body updated to state the Fable Max audit provenance, the AI-audit-not-GitHub-review caveat, first-match row 2, the un-re-audited D53 head, and that no amendment/R6/repair/rerun is authorized; PR flipped to Ready after a mechanical preflight (changed-file inventory Notes-only; evidence/ledger hashes unchanged; 1503 files retained; v1.18 absent; no live packet; tree clean; branch current with `main`; GitHub MERGEABLE/CLEAN). **STOP before merge.** D45 remains permanently UNVALIDATED_ATTEMPT; the reserved v1.18 result path stays absent. No replacement protocol, Della, M2d, or broader D19 planning begun.

## D54: M2cR current-tree cascade RETIRED to historical Git anchor 76f3c39 — isolated production verifier + closed-world anchor record + two standing tests re-anchored; hermetic, no compute; unblocks A7 without touching E3/L2/ledger/evidence/D53 — 2026-07-20

**Milestone.** Author-cast D54 authorization (post-M2cR Option 3, narrow "retire"): reclassify the four-manifest M2cR "current-tree" cascade as immutable historical artifacts frozen at their exact generating Git object, verified by retrieving pinned blobs from that commit instead of the live working tree, so future working-tree edits (the deferred A7 `experiments/d19_bench.py` rework, D55) can no longer desync it. Documentation / code / tests only; no scientific compute, no Della, no `--execute`. Does NOT begin A7/D55 and does NOT touch `experiments/d19_bench.py`.

**Problem.** `experiments/d19_bench.py` is hash-pinned as a worktree entry in the committed importable manifest (`3fde66c1…`), and the standing check `test_committed_importable_manifest_worktree_entries_match_tree` walked all 156 worktree entries against on-disk source under default `pytest -q` (verified: not `skipif`-gated). Any A7 edit to `d19_bench.py` would fail that check and silently desync a committed current-tree freeze. The four current-tree manifests (importable → env-freeze → infrastructure; + protocol) were generated together and self-consistent with the working tree; keeping them live-tree-bound taxes every future repo edit.

**Author decisions (cast 2026-07-20).** A: retire the WHOLE four-manifest cascade at anchor `76f3c39`. B: a small production-side verifier module, isolated (honest scope below), directly invocable, test-covered, altering no runtime verification default for any fresh arc. C: a canonical closed-world committed JSON anchor record under `docs/m2c_freeze/`. D: own D54 Draft PR; A7 stays a later D55; `d19_bench.py` untouched here. Lifecycle rule: the retired v1 files are immutable historical artifacts — any future execution arc mints new versioned manifest paths + its own live-tree verification, never regenerating or overwriting v1.

**Historical anchor (verified read-only).** `76f3c39631d8724c45db5f0e679608bffde39bbe` (= L2, the post-execution certification freeze) is the exact generating object: all four cascade artifacts were last committed there, and all 156 worktree importable entries + the four manifest digests reproduce byte-for-byte from that commit (0 missing / 0 mismatch). Distinct from the E3 execution freeze `367667f77881469b47c901345015c72b70e2fc02` (env `8b6795e5…`, infra `d8a3f302…`, protocol `ace374a6…`, v1.17 canonical `65381bc7…`), which stays immutable and reproduces from `367667f`.

**Decision (implementation).**
- NEW `bistar_gp/m2cr/historical_anchor.py` — isolated verifier (stdlib + read-only `git cat-file` only). Distinguishes four provenance classes: (i) E3 execution freeze at `367667f` (env/infra/protocol reproduce and are distinct from L2; v1.17 canonical attested, NOT recomputed); (ii) L2 cascade anchor at `76f3c39` (four manifests + 12 code + 8 artifact + 2 R1-schema infra internal pins + 156 worktree importable entries verify against anchor blobs; 39,812 environment-root entries labelled interpreter-attested and NEVER git-verified); (iii) committed R4 ledger/evidence (confirms the R4 grant `m2cr-ev-000006`/`m2cr-auth-20260719-03` `frozen_chain` is E3-bound; defers evidence integrity to the existing audit stack; no evidence rehash, no `payload.json` interpretation); (iv) future live-tree manifests (out of scope; `AnchorScopeError`). A separate `verify_v1_immutability` disk guard catches any regeneration/overwrite of the four v1 files.
- NEW `docs/m2c_freeze/m2cr_current_tree_anchor_v1.json` — canonical closed-world record, 2234 bytes, sha256 `1b4f8247cff7d184ae25802cdd993ee3cbd2b9145f8253af914995ba981301d0`, no trailing newline; + NEW closed Draft-2020-12 schema `m2cr_current_tree_anchor.schema_v1.json`.
- CHANGED `tests/test_m2cr_infrastructure_manifest.py` — the two former live-tree standing checks re-anchored to `76f3c39` via the verifier (accessed through its isolated file-path interface), renamed `test_retired_infrastructure_manifest_verifies_at_historical_anchor` / `test_retired_importable_manifest_worktree_entries_verify_at_historical_anchor`, the `M2CR_ALLOW_MISSING_COMMITTED_MANIFEST` bypass removed; every fresh-builder/live-tree test preserved.
- NEW `tests/test_m2cr_historical_anchor.py` — positive + robustness (a dirtied `d19_bench.py` in a scratch worktree at the anchor does not break content verification) + 11 negatives N1–N11.

**Isolation (Option A, honestly amended per author refinement).** The verifier SOURCE imports only the standard library; standalone file-path loading and direct script execution in a fresh interpreter load none of `bistar_gp`, `torch`, `gpytorch`, `pyro`, or the M2cR launch/bootstrap/capture/audit modules (the D54 tests exercise it ONLY through that isolated `importlib.util.spec_from_file_location` / subprocess interface). An ordinary package-qualified import first executes the pre-existing eager `bistar_gp/__init__.py`, which may load the scientific stack — existing package behaviour, not a dependency introduced by this verifier, and out of D54 scope to change; `bistar_gp/__init__.py` was NOT modified. No claim is made that every import spelling avoids the scientific stack; only the tested standalone interface and the absence of dependency edges in either direction. Verified: N10 tests assert stdlib-only source, standalone/direct-exec load pulling none of the forbidden set, and no reverse import.

**Reproducibility preserved (unchanged).** The closure/evidence/chain audits (`verify_chain`, `verify_ledger_against_evidence`, `derive_closure_events`) bind to the E3 grant `frozen_chain` (retrievable from `367667f`) + committed ledger/evidence bytes, none of which depend on the current-tree cascade's live-tree status. No pre-existing R2 infrastructure module changed vs the PR #20 base merge `5e0bbddc…` (N11 asserts the `bistar_gp/m2cr/` diff is empty apart from the new `historical_anchor.py`); `verify_infrastructure_manifest` and the capture/bootstrap importable-drift walk keep live-tree resolution for future fresh arcs. E3, L2, the committed R4 ledger/evidence (1503 files), and D53's PRESERVE_STOP disposition are untouched.

**Verification.** Focused: `tests/test_m2cr_historical_anchor.py` 27 passed; `tests/test_m2cr_infrastructure_manifest.py` 14 passed (12 preserved fresh-builder + 2 re-anchored). Full suite `python -m pytest -q`: **1045 passed, 2 skipped** (the 2 pre-existing opt-in skips). Provenance: hermetic tests exercise the isolated verifier, read-only git objects, fake/synthetic manifests and ledgers, a git-worktree scratch checkout at the anchor, and the pre-existing fresh-builder infrastructure launches; NO A7 benchmark, Della job, real Mauna/holdout computation, M2cR payload, or new scientific/model/profile execution ran.

**Review.** One fresh bounded read-only Codex xHigh review (model `gpt-5.5` via the use-codex skill — the codex CLI's current model; the authorization's "GPT-5.6-sol" label is a naming difference, flagged to the author). Read-only honored (zero edits, checksum-verified). Verdict **REVISE** with 4 MAJOR conforming defects, all independently confirmed and closed in the single authorized correction pass: (1) the anchor schema now `const`-binds the exact commits, retired paths, and manifest/E3 sha256s, so a canonical-but-wrong record no longer validates; (2) `verify_r4_ledger_e3_binding` pins the grant identity (`m2cr-ev-000006` / `m2cr-auth-20260719-03`) and requires EXACT `frozen_chain` equality (no extra/missing members); (3) the `verify_v1_immutability` docstring narrowed to the accurate claim (it reads only the four retired manifest files; `load_anchor_record` / `verify_r4_ledger_e3_binding` read the record / ledger); (4) worktree-entry verification now also checks `size` (parity with the retired live-tree check) and rejects unrecognised roots. +5 discriminating negatives added (size mismatch; unrecognised root; wrong-identity grant with correct chain; extra `frozen_chain` member; well-formed-but-wrong commit/path/hash). Per the authorization no second Codex round was run; the fixes were self-verified (focused + full suite green).

**Status.** Draft PR pending. HARD STOP before Ready / merge / any local timing run / Della / A7 / D55 / M2d / new science arc. `experiments/d19_bench.py` untouched. D45 remains permanently UNVALIDATED_ATTEMPT; the reserved v1.18 result path stays absent; D53 PRESERVE_STOP unaltered.

### D54 — Update 1 (2026-07-20): exact-head acceptance audit APPROVE; final review lineage (GPT-5.5 REVISE → one correction pass → GPT-5.5 APPROVE); Notes-only Ready closure

**Review lineage (recorded precisely; GPT-5.5 throughout — neither review was GPT-5.6-sol).** The single bounded implementation review was Codex **GPT-5.5** (xHigh, read-only); it returned **REVISE** with **four** confirmed conforming findings (schema const-binding of every frozen identity; `verify_r4_ledger_e3_binding` grant identity + exact five-member `frozen_chain` equality; the `verify_v1_immutability` live-tree docstring accuracy; worktree-entry `size` check + unrecognised-root rejection). **One bounded correction pass** closed all four and added five discriminating negatives; no second review round was run. The final, strictly read-only, exact-head acceptance audit — Codex **GPT-5.5**, read-only, at reviewed code head **`9efeb76`** — returned **APPROVE**, independently confirming all four findings closed and every acceptance criterion met. The cast authorization used the label "GPT-5.6-sol"; the codex CLI's actual model was gpt-5.5, and both the review and the acceptance audit ran on gpt-5.5.

**Independent re-verification at `9efeb76` (read-only, this session).** The diff is exactly the seven-file allowlist; the protected surface (12 R2 modules, the four retired manifests, ledger/evidence, `experiments/d19_bench.py`, v1.17/v1.18) is absent from the diff; the four retired manifests reproduce `disk == blob == record` at anchor `76f3c39`; the isolated verifier CLI returns `ok:True` (156 worktree git-verified / 39,812 environment interpreter-attested / infra pins 12·8·2 / immutability ok / R4 grant `m2cr-ev-000006` · `m2cr-auth-20260719-03`); the anchor record is `1b4f8247…` (2234 bytes, single-line canonical); `gtoy_profile_result_v1.18.json` is absent; the tracked tree is clean.

**Ready closure (Notes-only tail).** This D54 Update plus the SCRATCHPAD alignment are the ONLY change after the reviewed code head `9efeb76`; the reviewed code (the verifier `historical_anchor.py`, the anchor record, its schema, and both test files) stays byte-unchanged. After this Notes-only commit: the authorization's `python -m pytest -q` is run (require 1045 passed / 2 skipped / 0 failed) and the mechanical Ready preflight is performed, then PR #21 is flipped to Ready with a body that distinguishes the reviewed code head `9efeb76` from the Notes-only tail. **HARD STOP before merge / A7 (D55) / editing `experiments/d19_bench.py` / benchmark / Della / M2d / scientific computation.** D45 remains permanently UNVALIDATED_ATTEMPT; the reserved v1.18 result path stays absent; D53 PRESERVE_STOP unaltered.

---

## D55: A7-first — `experiments/d19_bench.py` reworked to the v1.2-point-6 timing-only firewall; closed-world path-aware schema; pre-import four-variable thread contract; new A7 output namespace; pre-D22 anchors superseded; hermetic, NO benchmark run — 2026-07-20

**Milestone.** Author-cast D55 authorization (Option-C / A7-first). Prereg addendum v1.8 item 2(d) requires `experiments/d19_bench.py` be reworked to the v1.2-point-6 timing-only firewall and its pre-D22 anchors marked superseded; v1.8 item 5 gates the A7 Della re-benchmark on that rework plus a key-inventory audit. D55 performs the rework only. HERMETIC: no benchmark run (local or Della), no thread sweep, no addendum with measured values, no science. D19 stays OPEN; Option B (a new preregistered arc to unblock the M2c science) remains deferred. D53 PRESERVE_STOP untouched.

**Why this is now safe.** D54 (merge `7277b421`, PR #21) retired the M2cR current-tree cascade to historical anchor `76f3c39`, so `experiments/d19_bench.py` is verified against a pinned Git blob rather than the live tree. Independently re-verified before editing: the live file hashed to exactly the pinned `D19_BENCH_SHA_AT_ANCHOR = d8a606fc…` / size 12824, and the standing checks read anchor blobs. **Demonstrated after editing:** the live file now diverges (`3be8083d` ≠ anchor `ec8bebbf`) and `tests/test_m2cr_historical_anchor.py` + `tests/test_m2cr_infrastructure_manifest.py` still pass (41). D54's premise held under the first real A7 edit, not merely a synthetic one.

**Authority for what may be persisted.** v1.2 point 6 (timing, potential/gradient evaluation counts+costs, leapfrog counts) as EXTENDED by the v1.6 item 6 "firewall reading" (run configuration — seed, iteration counts, tree depth — plus environment provenance — versions, threads, platform, host, timestamp, git sha — plus fixed caveat strings; AST-audited key inventory; exception-message suppression), RATIFIED by v1.9 item 1 and v1.11 item 4 (leapfrog counts serve aggregate engineering cost only). Note v1.6 names **step size** as forbidden despite looking configuration-like: it is adapted, hence an inference output.

**Violations corrected (verified line-by-line against the pre-rework file).** It loaded the holdout-bearing `load_mauna_loa` rather than `load_mauna_loa_training`; persisted `map_hyperparameters` (7 values) and `map_noise_variance_normalized`; persisted `hessian.eigenvalues`/`min_eig`/`max_eig`/`spd`/`n_below_1e-6_floor`/`condition_number_after_floor`; read a forbidden persisted value back out to seed the profile step; wrote raw exception text via `provenance.raw_fetch_error`; printed via a path that bypassed any schema check; and had no sanitized error path at all.

**Author ballot (cast 2026-07-20).** C1 PARTIAL PERMIT (scalar `n_sampled_sites` + `dim_unconstrained` only; site names and the period boolean stay internal fail-closed checks). C2 DROP (every `y_*` statistic, `x_offset_years`, x min/max, `span_years`). C3 CLOSED LIST (OpenML id 41187, `source="vendored"`, the loader-RETURNED canonical year/month/co2 sha256, training-point count — no network, no co2-only recomputation, no holdout count, no split rule, no raw/monthly counts). C4 DROP (stride-month means, calendar coverage). C5 NARROW PERMIT (aggregate `n_failed_potential_evals` only). C6 DROP (no eigendecomposition at all; Hessian construction timed only). T1 AUTHORIZE ALL FOUR thread controls. Plus: no sampler, no leapfrog field, no new workload; `single_thread` block removed; module scope stdlib-only; final-only persistence; no `--synthetic` CLI mode.

**Decision (implementation).**
- **Pre-import thread contract.** `--threads ∈ {1,2,3,4}` required. `OMP_NUM_THREADS`, `MKL_NUM_THREADS`, `OPENBLAS_NUM_THREADS`, `VECLIB_MAXIMUM_THREADS` all set before importing numpy/torch/gpytorch/pyro; then torch intra-op set and verified. `thread_configuration_checks_passed` certifies ONLY: heavy modules absent at env-set time, the four env values equal to the request, torch intra-op equal to the request. `THREAD_SCOPE_NOTE` states explicitly that it does NOT certify actual BLAS worker counts, torch inter-op, a process-wide ceiling, or exact physical workers, and that `VECLIB_MAXIMUM_THREADS` is a maximum, not a guarantee. Inter-op is OBSERVED, never set. Verified read-only that local NumPy 1.26.4 is built against Apple **Accelerate** (not MKL/OpenBLAS), which is why the honest scope note matters.
- **Closed-world path-aware schema.** Every key always present; absence expressed as `null` gated by a `status` enum. Unknown keys, valid keys at the wrong nesting level, NaN, and ±inf all rejected with full dotted paths. Cross-field rules X1–X10 plus a stage-1 failure bound and an ISO-8601-UTC check. `$.timings.log_joint` renamed `$.timings.potential_value_eval`.
- **Two-layer key denial** (author-ratified after the orchestrator found the defect): `FORBIDDEN_EXACT_KEYS` for prohibited names that are substrings of ratified ones (`sampled_sites` of `n_sampled_sites`, `map_*` of `map_fit`, `ess` of `hessian`), `FORBIDDEN_KEY_SUBSTRINGS` only for families colliding with nothing. No exemption list. Honestly documented as defence in depth that cannot be complete; the closed-world schema is the actual guarantee.
- **Output capture.** FD-level `os.dup2` over the heavy imports AND the whole workload, restored in `finally`; Python-level `redirect_stdout` deliberately NOT used (native BLAS/C-extension writes bypass it). Warnings captured with `record=True`, reading only `category.__name__`, bucketed to a frozen enum with `"other"` catch-all; messages never read. Residual limitations documented in the module docstring: shutdown-time output after restore, a native library caching an fd before install, faulthandler/fatal-signal output, and argparse output before install.
- **Final-only atomic persistence.** No incremental checkpoints. Validate once, render and scan the stdout report, THEN `os.replace` from a same-directory temp file. On any failure nothing is written and an existing artifact is untouched. Honest overwrite preserved for the same target; no no-clobber policy.
- **A7 output namespace (author correction).** `runs/d19_a7_timing/bench_{scale}_threads_{threads}.json`. The old default addressed the two superseded anchors directly, so an A7 run would have destroyed the provenance the supersession record preserves; and omitting threads would have let a sweep overwrite its own rows. `reject_superseded_target()` refuses either legacy path before any heavy import, data load, or model construction.
- NEW `docs/d19-pre-d22-bench-supersession.md` — per-artifact supersession record. `bench_sub.json` sha256 `ed5c7bf4…` (3663 B, 19 prohibited keys); `bench_full.json` sha256 `ea47270d…` (3451 B, 17). Both additionally carry `provenance.n_test_months`, holdout-derived metadata from the `load_mauna_loa` call — a SECOND disqualifying defect beyond the one v1.8 2(d) names. Neither file deleted or edited; deletion is flagged as a SEPARATE author decision.

**Review lineage (recorded precisely).** Implementation: Codex **gpt-5.6-sol** at xHigh — verified from the session-log `payload.model`, NOT merely the CLI flag, because D54's authorization label said "GPT-5.6-sol" while the CLI actually ran gpt-5.5. This time the exact authorized model ran; no substitution. ONE strictly read-only review, Codex **gpt-5.6-sol** xHigh, verdict **REVISE**: 1 BLOCKER, 6 MAJOR, 2 MINOR. Every finding independently verified before action; the BLOCKER empirically (a probe showed `SystemExit("noise=0.123456")` escaping `_firewalled_main` with its argument intact, which the interpreter prints verbatim at exit; after the fix the same probe shows bare `(1,)`). ONE bounded correction pass closed all nine plus both author corrections. Two PRE-review conforming corrections (the two-layer restructure and loader-returned provenance) were ruled by the author to be implementation corrections, leaving the single post-review pass unspent for the review findings.

**Findings closed.** F1 BLOCKER post-parse `SystemExit` leak → `parse_args`/`run` split, argparse exits stay readable (CLI tokens only), post-parse `SystemExit` sanitized to class name. F2 forgeable `_ValidatedRecord` + single-layer report scan → token-gated constructor, exact report key inventory, both denial layers. F3 FD cleanup could skip restoration → best-effort flush, acquisition inside `try`, independent per-fd restore/close. F4 failure after commit left the artifact → render and scan before `os.replace`. F5/F6/F7 non-discriminating tests → runtime import-spy wiring tests, post-exit and post-exception fd writes, `os.replace` spy, recursive missing-key deletion, one-defect-at-a-time ordering and warning mutations, X4–X7 individually. F8 exact layer missed `map_values`/`ess_mean`/`per_site_value` → enumerated. F9 drift test imported torch → AST-parses `data.py`; suite runtime fell 1.82s → 0.10s, confirming the import is gone.

**Beyond the review.** A new mismatch test revealed that `bistar_gp` was IMPORTED before the torch thread verification (the reviewer's "before the loader CALL" was accurate, but weaker than intended). Verification moved to immediately after `import torch`, so a failed thread contract aborts without importing any project module.

**Verification.** Focused `tests/test_d19_bench_firewall.py`: **63 passed** (41 as specified, expanded during the correction pass). Full `python -m pytest -q`: **1108 passed, 2 skipped, 0 failed** (1:32:53) against the D54 baseline of 1045/2 — 1045 + 63, zero regressions. Provenance, stated precisely: the D55 tests are hermetic (fakes, spies, import spies, synthetic scalar callables, AST scans); the full-suite run additionally executes the repository's PRE-EXISTING hermetic tiny-E1 sampler regressions and synthetic/fake infrastructure tests, which predate D55 and are unchanged by it. **NO A7 benchmark, Della job, thread sweep, real Mauna or holdout load, M2cR payload, or new scientific/model/profile computation ran.**

**Changed files (exactly five).** `experiments/d19_bench.py` (rework), `tests/test_d19_bench_firewall.py` (new), `docs/d19-pre-d22-bench-supersession.md` (new), `Notes/DECISIONS.md`, `Notes/SCRATCHPAD.md`. Protected and untouched, confirmed absent from the diff: `bistar_gp/**` (incl. `m2cr/historical_anchor.py`), the D54 anchor record and schema, the four retired manifests, `tests/test_m2cr_*.py`, M2cR ledger/evidence, D53 PRESERVE_STOP, prereg v1.17/v1.18, `docs/prereg-addenda-d19.md`, `experiments/d19_e1_bench.py`, and every existing file under `runs/` (never staged).

**Status.** Draft PR pending. **HARD STOP before Ready / merge / any local timing run / Della / thread sweep / measured-value addendum / M2d / scientific computation.** The A7 timing run (local, then thread-pinned on Della) and the thread-pinning addendum are SEPARATE later authorizations. D45 remains permanently UNVALIDATED_ATTEMPT; the reserved v1.18 result path stays absent; D53 PRESERVE_STOP unaltered. Deleting the two superseded artifacts remains an OPEN separate author decision.

**D55 Update 1 — exact-head acceptance audit APPROVE; the one residual closed by literal pinning (2026-07-21).**

**Audit.** A final independent, strictly read-only acceptance audit ran at exact head `dd673d9` (base `7277b421`) and returned **APPROVE**. **Provenance, stated precisely: this audit was performed by the orchestrator, Claude Opus 4.8 — NOT by GPT-5.6-sol.** GPT-5.6-sol performed the EARLIER bounded implementation review, which returned REVISE; it was not re-invoked, and no second reviewer was launched. Recording this distinction explicitly because D54's cast authorization carried a model mislabel, and the same error must not enter D55's record.

The audit independently re-derived rather than trusting summaries: diff scope exactly the five files over one commit; **21 of 21 adversarial injections rejected** (forbidden keys at depths 1-3 and inside list elements, exact-layer names, unknown keys, valid-key-wrong-path, non-string mapping keys, NaN/±inf, negative timings, tampered constants, null `git_sha`, off-by-one `n_train_points`, disagreeing thread fields); `_ValidatedRecord` unforgeable through public paths and the writer refusing a plain dict while leaving an existing artifact byte-intact under both failure modes; the bare holdout loader absent and `eigvalsh`/`eigvals`/`slogdet`/`get_samples`/`NUTS`/`MCMC`/`fit_hmc` all at zero occurrences; the supersession hashes/sizes/`n_test_months` recomputed from disk and matching the document; `runs/` contributing zero lines to the diff.

**Two informational NOTEs, both adjudicated NOT actionable against v1.2 point 6 / v1.8.** (i) `import gpytorch` transitively initializes torch before the explicit `import torch`, but the four environment variables are already written and verified before the capture block and hence before ANY heavy import; `torch.set_num_threads()` is effective post-import; the result is explicitly verified and aborts on mismatch, making the sequence self-checking; and no `bistar_gp` import, data load, or model construction precedes that verification. No concrete contract violation. (ii) `_atomic_write_json` trusts the validated marker rather than re-validating, and the private `_VALIDATION_TOKEN` could be reached by a caller deliberately using private names — outside the "ordinary public code paths" criterion.

**The one residual (MINOR), now CLOSED.** `FIREWALL_NOTE` and `DESIGN_LABEL` were validated by `Const(<module constant>)` while `_valid_record()` built those fields from the SAME module constant, making the comparison tautological with respect to the TEXT: measured 1 mention / 0 content assertions each. A revert of the author's correction-2 wording — the correction that stopped the note from falsely claiming every sample is discarded unread — would have left the whole suite green. `SCHEMA_KIND` was already literal-pinned and `THREAD_SCOPE_NOTE` already carried three substring assertions, so the gap was confined to those two constants. This means the earlier claim that the bounded correction pass closed "all nine" findings was **inaccurate at the time**: the previous review's F7 had six sub-items and five were closed, this one was not.

**Correction (tests only; no production constant changed, no schema field renamed).** `tests/test_d19_bench_firewall.py` gains five tests pinning both constants to **independently transcribed literals** (exact equality plus length), semantic assertions encoding the three author-ratified claims (posterior samples and disallowed diagnostics discarded unread; transient prior proposals and MAP-derived values read internally ONLY to execute the frozen timing workloads; none printed or serialized), explicit rejection of the superseded overclaiming phrasings, phrase-level pinning of `DESIGN_LABEL`, and a guard-the-guard test asserting the expected literals are not re-sourced from the module. Verified discriminating by mutation: the unchanged control passes while all six mutants fail — full revert, three distinct weakenings, a same-length single-word swap, and a `DESIGN_LABEL` rewording. **With this, all nine prior review findings (1 BLOCKER, 6 MAJOR, 2 MINOR) are fully closed**, together with both author corrections.

**Verification.** Focused `tests/test_d19_bench_firewall.py`: **68 passed** (63 + 5). Full `python -m pytest -q`: **1113 passed, 2 skipped, 0 failed** (1108 + 5; the D54 baseline was 1045/2, so D55 contributes 68 tests and zero regressions). No further model review was run — the audit's APPROVE stands as the final review, and this correction is tests-and-Notes only. No benchmark, Della, thread sweep, `runs/` write, M2d, or scientific computation.

## D56: A7 execution-protocol freeze (docs-first) — Della recon, ballot AC1-AC11, frozen submit script + fail-closed validator; hermetic, NO benchmark run — 2026-07-21

**Milestone.** D56 freezes the mechanical A7 Della execution protocol before any A7 execution. Authority is `docs/plan-d19-mauna.md` §1.3/§7 A7; preregistration v1.2 point 6, v1.5 owed-numbers, v1.6 item 6, v1.8 items 2(d)+5, v1.9 item 1, and v1.11 items 4+6; D25's user-executed boundary; D55's reworked and now-frozen benchmark vehicle; and the author ballot cast 2026-07-21. Preparation only and hermetic: documents, submit script, validator, and synthetic tests; no local or Della benchmark, data load, project CLI, or network action. The eventual A7 run is USER-EXECUTED on Della under a separate byte-exact authorization.

**Recon (read-only, 2026-07-21).** Through the author-opened SSH master socket: `della9`, RHEL 9.8, slurm 25.11.6; account `suyoghc`, partition `cpu`, and the live account/partition/QOS facts. Live `man sbatch` established that variables cannot be combined with `--export=NONE`, so the SHA is positional; `NONE` invokes get-user-env and makes `module` available, while `NIL` does not. The `cascade` token spans 107 heterogeneous nodes, including 13 bigmem `della-h12n[1-13]`, optane `della-h17n1`, mixed `della-h12n17`, and Sapphire Rapids `della-h12n18` plus `della-i13n25`; the exclude list leaves the exact 90-node `intel,cascade,rh9` pool. `della-h16n*` is now `amd,genoa,rh9,nvme`, so D3/D8's `della-h16` history attaches to retired hardware and remains margins-only. The existing `/scratch/gpfs/SUYOGHC/bistar_gp_c` clone is STALE+DIRTY (HEAD `865182a5`, 2026-07-03, `fix/laplace-zmx`, four dirty paths) and is preserved untouched. Conda environment `bistar_gp` is frozen at python 3.11.14, torch 2.10.0+cu128, gpytorch 1.15.1, pyro 1.9.1, numpy 2.4.2; numpy differs from local 1.26.4, the local suite has never run under 2.4.2, and drift or any sanitized failure aborts before cell 1. There is no site-packages `bistar_gp`; `PYTHONPATH` and an in-worktree containment check are mandatory.

**Author ballot (cast 2026-07-21).** AC1 NONE (no local run; (sub,1) is the Della smoke cell). AC2 all eight cells. AC3 --budget-s 600 uniform; skipped_for_budget / extrapolated_from_stage1 are truthful cell-valid statuses, surfaced explicitly, never described as measured. The AC3 budget is an operational ceiling. AC4 Model B (one job, one node, cpus-per-task=4, strictly serial). AC5 STOP-only (no automatic retry or completion; further invocations need a new explicit author authorization). AC6 docs-first D56 with committed submit script + validator. AC7 v1.8-item-5 key-inventory audit discharged by D55. AC8 scp into an ABSENT local staging dir, dual-source hash cross-check, then local commit. AC9 option (ii): --constraint=cascade + the exclude list above + in-job exact ActiveFeatures check "intel,cascade,rh9" before cell 1 + all claims scoped to the frozen 90-node pool. AC10 environment frozen (incl. numpy 2.4.2), no modification, drift fails before cell 1. AC11 no SLURM mail directives. The exact AC9 exclude list is `della-h12n[1-13],della-h12n[17-18],della-h17n1,della-i13n25`.

**Decision (implementation).** Four new files: `experiments/submit_d19_a7_bench.slurm`, `experiments/d19_a7_validate.py`, `tests/test_d19_a7_protocol.py`, and `docs/d19-a7-execution-protocol.md`. The submit script receives the execution SHA positionally under `--export=NONE`; requires the exact chdir; separates tracked/index cleanliness from a closed-world untracked/evidence-directory check; enforces the interpreter, conda prefix, exact five versions, import containment, and exact ActiveFeatures through explicit exit codes 64–83; and runs the frozen scale-major eight-cell matrix strictly serial with STOP on the first failure and no retry. Its `16G`, `02:00:00`, and uniform budget are operational ceilings, not predictions. Protocol P0–P7 covers GitHub-auth-or-bundle retrieval, collision refusal without removal/reset/clean/reuse, detached worktree identity and total cleanliness, no-`-p` evidence creation, one submit, `sacct -P`, complete `PROVENANCE.sha256`, absent-destination and dotfile-safe directory transport, and three-way in-job / manifest / independent-local hash agreement before validation or movement; all partial/failure evidence is preserved. The stdlib-only validator imports only `validate_record`, `FirewallViolation`, and the two forbidden tables from the frozen vehicle via file-location loading; V1–V15 enforce the exact file set, strict JSON, vehicle firewall, independently transcribed key inventory, forbidden scans, finite/nonnegative fields only, filename/config/thread/version/SHA agreement, the frozen node scope, log order, scheduler truth, timezone-aware timestamp bounds, three-way provenance hashes, and the two superseded-anchor bytes, then prints the truthful status census. The commit topology is reviewed `H` (or `H'` after one bounded correction pass) → optional Notes-only tail → `M56`; launch additionally requires `origin/main == M56`, second parent = PR head, and an empty `git diff H'..M56 -- experiments/ bistar_gp/ tests/ docs/d19-a7-execution-protocol.md`, with script-enforced worktree SHA and validator-enforced artifact SHA.

**Verification.** Focused `tests/test_d19_a7_protocol.py`: **22 passed**. Frozen `tests/test_d19_bench_firewall.py`: **68 passed**. Both are hermetic: synthetic evidence, local bytes, AST/source checks, and no scientific dependency import. `experiments/d19_bench.py` and `tests/test_d19_bench_firewall.py` are untouched; `runs/` is untouched; no file outside the six-file D56 allowlist is changed.

**D56 Update 1 — two-reviewer gate REVISE -> single bounded correction pass (2026-07-21).** Implementer: Codex gpt-5.6-sol xHigh, verified from the session-log model field. Reviewer 1: FRESH Codex gpt-5.6-sol xHigh, read-only, verdict REVISE (2 BLOCKER, 6 MAJOR, 2 MINOR). Reviewer 2: Opus 4.8 (model id `claude-opus-4-8`, transcribed from its own context), verdict REVISE (1 BLOCKER, 1 MAJOR, 3 MINOR). The shared V8 inter-op BLOCKER was real: the fixture masked it by setting inter-op equal to threads even though the vehicle observes and never sets inter-op. Orchestrator cross-verification confirmed the findings closed by FIX1–FIX13; it REJECTED Opus's scan-surface MINOR as not actionable because narrowing the scan would weaken leak detection and fail-closed with author reading is intended. It also recorded that Codex's “stale before cell 1” ordering claim was inaccurate—the closed-world check precedes any import—while the gitignored `sitecustomize.pyc` import-hijack vector was real and is now shell-guarded with `python -B`. This correction pass is the SINGLE authorized pass. Final verification: **38 passed** focused and **68 passed** frozen firewall.

**D56 Update 2 — focused re-confirmation at H′; H′ is the reviewed head (2026-07-21).** Heads: H = `f2628fdc` (implementation, reviewed), H′ = `4c9b79ae` (the single bounded correction pass; delta = the six allowlist files only). Infrastructure note, recorded for lineage accuracy: a harness restart killed the FIRST launch of the correction pass with ZERO bytes landed (working tree verified byte-identical to H afterward), so the relaunch that produced H′ is the same single pass, not a second one; the same restart destroyed the original Opus review session's transcript. Focused re-confirmation, scoped strictly to the corrected findings: (1) Codex — a FRESH gpt-5.6-sol xHigh read-only session (identity verified from the session-log model field) — verdict **CONFIRMED-CLOSED**, no residual, no new delta defect. (2) Opus — a FRESH `claude-opus-4-8` session (identity transcribed from its own context) seeded with the original five findings VERBATIM because the original session could not be resumed — findings 1–4 **CLOSED** (with independent re-verification of each closure, including that the fixture's inter-op=4 makes the valid-evidence test discriminate against the old V8), finding 5 **ADJUDICATION-ACCEPTED**, and **DELTA-CLEAN** after a hunk-by-hunk pass over all six changed files. Per §7 of the protocol, H′ thereby becomes the reviewed head. These are AI reviews, not GitHub reviews or CI. This Update is Notes-only; the reviewed code at H′ is byte-unchanged.

**D56 Update 3 — Ready-gate full suite found a pre-existing guard incompatibility; authorized test-only micro-correction (2026-07-21).** The Ready closure's first `python -m pytest -q` at tail `5c10701` aborted during COLLECTION: the pre-existing M2a guard `tests/test_slurm_argparse.py` shlex-parses `experiments/*.slurm` line by line, and the D56 submit script is the first committed Slurm job using POSIX backslash-newline continuations (exactly two, lines 40 and 101) — a trailing lone backslash raises `ValueError: No escaped character` inside the parametrize decorator, so the suite never ran. Additionally characterized before fixing: a naive crash-only fix would have left the guard seeing the continued invocation with ZERO flags (vacuously green). Author authorized Option A: ONE micro-correction to `tests/test_slurm_argparse.py` only (outside the D56 six-file allowlist; reviewed D56 execution/validator bytes at H′ byte-unchanged; no new broad review). Implemented by the orchestrator and independently inspected: `_logical_lines()` joins odd-trailing-backslash continuations (even runs are escaped backslashes; comment lines cannot start a continuation), preserves the FIRST physical line number for ids/diagnostics, wraps shlex errors with `file:line` context, and a dangling continuation now fails with `<file>:<line>: dangling backslash continuation` instead of the opaque error. No file special-cased or excluded. Five discriminating tests added; the parametrization now includes `submit_d19_a7_bench.slurm:101-experiments/d19_bench.py`, whose passed flags are asserted to be exactly `--scale/--threads/--budget-s` and which passes the argparse cross-check against the frozen vehicle. Test-only commit `b50350e`. **Verification at `b50350e`:** guard `tests/test_slurm_argparse.py` **12 passed** (was 6: 5 invocations + coverage; now + our invocation + 5 discriminating); focused **38** (D56 protocol) and **68** (D55 firewall) unchanged; full `python -m pytest -q` **1157 passed, 2 skipped, 0 failed** (0:05:46) — reconciles exactly as 1113 (D55 baseline) + 38 + 6. Provenance: hermetic throughout (synthetic evidence, source/AST checks, the pre-existing hermetic tiny-E1 sampler and synthetic infrastructure regressions); NO A7 benchmark, Mauna load, holdout access, Della execution, or new scientific run. The cumulative PR allowlist is now SEVEN files (the six D56 files + `tests/test_slurm_argparse.py`).

**D56 Update 4 — D56a docs-only amendment: the frozen §7 launch closure was self-blocking; replaced by the explicit two-anchor rule (2026-07-21).** The separately authorized test-only guard commit `b50350e16f5a9356c383e09c357df258f99cd432` made the frozen broad `H'..M56` diff permanently non-empty even though the execution surface remained byte-identical to `H'`; the author-cast D56a instruction therefore authorizes this three-file docs-only allowlist and no new decision number. The repaired rule requires execution-byte closure to `H'`; the exact known test-only delta `tests/test_slurm_argparse.py`, pinned to blob `5ef26ec2464aeb9e788a6e14b443e5801ed8b5c8`; amendment closure to reviewed head `A`; and true-merge, second-parent topology at launch anchor `M56a`, with the unchanged script, validator, artifact, and V0 bindings now anchored at `M56a`. It does not reinterpret the earlier two-reviewer review or its focused re-confirmation, alters no execution or validator byte, and authorizes no benchmark or Della access. Mechanical proofs recorded at amendment time: current `origin/main` at D56 merge `66ca91cd8a5e0a2bbb7d984b2e5298707160d6c0` has an empty execution-surface diff from full `H'` `4c9b79ae8fbe42ceeacbeac1f99a2cc1599ece7a`; its complete `tests/` delta from `H'` is exactly the guard file; and that file's current blob equals the blob at `b50350e`. Verification: `python -m pytest tests/test_d19_a7_protocol.py -q`: **38 passed**; `python -m pytest tests/test_slurm_argparse.py -q`: **12 passed**; full `python -m pytest -q`: **1157 passed, 2 skipped, 0 failed** (0:05:17), identical to the pre-amendment counts as required for a docs-only change. Provenance: hermetic throughout; NO A7 benchmark, Mauna load, holdout access, Della execution, or new scientific run. Post-audit hardening (the single bounded docs-only correction pass): a fresh read-only Codex gpt-5.6-sol audit at amendment head `567f21f` returned one CONFIRMED BLOCKER — the rule left the interpreter's import surface open (a tracked top-level `platform.py`, new top-level package, or new `experiments/*.py` between `H'` and `M56a` would pass all gates yet shadow stdlib/scientific modules under `PYTHONPATH=$EXEC_ROOT` + the `experiments/` script directory, invisible to item 1 and V0). Closed by item 6, total-surface closure: the complete `H'..M56a` name-diff may contain only `Notes/`, `docs/`, and the blob-pinned `tests/test_slurm_argparse.py`. The same hole existed in the superseded broad rule (root-level paths were outside its pathspec too); the hardening makes the amended rule strictly stronger than the original.

**D56 Update 5 — launch authorization cast and SPENT; A7 attempt 1 (job 11485635) FAILED pre-cell; failed-attempt evidence committed byte-for-byte at an attempt-scoped path under a narrow author clarification (2026-07-21/22).** The byte-exact launch authorization naming M56a `e9457d719bfbaea9e890329687b8c166b872570a` was cast by the author 2026-07-21 23:43 -03 against revision 2 of the prepared launch packet (23611 B, sha256 `d5d225a41c182d81f474a5bb5116a652d6ed3175728068d6ac099fea8a6585f4`). Preparation lineage: a read-only preflight verified all six amended-§7 conditions PASS at M56a with full-SHA commands; ONE fresh read-only Codex **gpt-5.6-sol** xHigh audit of the packet returned **APPROVE, zero findings** (model identity verified from the session banner AND the rollout-log `"model"` field, sandbox read-only); revision 2 then added two author-identified, mechanically tested operational gates — a read-only pre-P6 `sacct` readiness loop protecting the single-shot P6 capture (protocol §4 makes rerun-when-present refuse-and-STOP, and V12 needs a terminal parent row, so `squeue` disappearance is not the P6 trigger), and an enforcing stdlib-only three-way hash gate before validation (protocol P7 requires in-job/manifest/independent-local agreement BEFORE the validator; the drafted `grep` only displayed). P0–P5 were then executed personally by the author on della9: P0 `ls-remote` PASS (remote HEAD = M56a; bundle fallback unused), P1 fetch + object-verify PASS, P2 collision guards negative then detached worktree `/scratch/gpfs/SUYOGHC/bistar_gp_a7_exec` at exactly M56a (2103 files), P3 HEAD/cleanliness PASS, P4 `mkdir runs/d19_a7_timing` silent success, P5 exactly ONE submission → job **11485635**. **Captured facts.** sacct parent row, verbatim: `11485635|FAILED|1:0|00:00:03|02:00:00|00:00.061||della-r3c1n8|2026-07-21T22:56:32|2026-07-21T22:56:44|2026-07-21T22:56:47` (della-r3c1n8 is inside the frozen 90-node pool; batch step FAILED 1:0, extern COMPLETED). stdout: exactly 0 bytes. stderr: exactly one 44-byte line, `environment: line 49: PS1: unbound variable`. No `PREFLIGHT-FAIL` marker, no `ENV-OK`, no `CELL` line, no artifact: no benchmark cell, Mauna load, or `bistar_gp` import occurred, and the persistence firewall was never reached. **Bounded inference, recorded as inference and not as identification.** The nounset diagnostic requires `set -u`, which only the submit script's line 18 enables, so the abort postdates line 18; fail(64)/(65)/(66) stayed silent and the first stdout write (line 85) was never reached, bracketing the death to the environment-setup region, lines 29–33 (`module purge` / `module load anaconda3/2024.6` / conda hook eval) — under `set -u` an unbound variable is a fatal shell error that exits a non-interactive shell past `||` guards, and line 29 is unguarded besides; exit status 1 matches. The file reporting itself as `environment` has **NOT** been identified; attribution is deferred to a separately authorized, strictly read-only Della diagnostic. **Evidence chain.** P6-readiness loop returned the terminal row; P6/P6b captured single-shot (a pasted-transcript duplication of the capture pair was adjudicated mechanically: neither no-clobber guard printed and the interactive session survived, so the redirects executed at most once — no truncation, no overwrite); a first P7 attempt mistakenly launched ON Della was aborted at the unknown-host-key prompt by answering `no` (no known_hosts write, no transfer, nothing mutated); P7 then ran correctly from the local Mac in dotfile-safe directory form into the absent staging path; `shasum -a 256 -c PROVENANCE.sha256` OK; the enforcing three-way gate **FAILED exactly as designed** on the failure set (the eight bench JSONs missing from the staging entries, manifest set, and in-job set; ZERO hash disagreements among present files); the §5 validator was correctly not run after the gate FAIL. **Pins (sha256, byte-for-byte).** `PROVENANCE.sha256` 253 B `c420d12425d6afa29e5b204f2ef47496ed7779485922403f88a7f883e9ca4b25`; `job_metadata.txt` 451 B `09f524460da0ba70e94d5b0ee42a67f11638346f3235f1f904baa44dc67deb37`; `slurm-11485635.err` 44 B `59be6fc2986c195cdedc5c28bf6f8a03f3cf6513244db911e8b0f049a3ebc9d1`; `slurm-11485635.out` 0 B `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855` (the empty-input SHA-256: stdout was empty at capture on Della, not lost in transport). **Narrow author clarification (cast 2026-07-22).** §8's allowlist B is the success-only twelve-file contract; it and canonical `runs/d19_a7_timing/` remain RESERVED for a future successful attempt (its `job_metadata.txt`/`PROVENANCE.sha256` names would otherwise collide across attempts). This failed attempt's four files are committed byte-for-byte at attempt-scoped **`runs/d19_a7_failed_11485635/`**, renamed from the staging path in exactly one `mv` with every pin re-verified after the move; `git add -f` needed only for the `.out` (`.gitignore:29` `slurm-*.out`). Changed files exactly seven: the four evidence files plus DECISIONS/SCRATCHPAD/CHATLOG. Branch `evidence/d19-a7-failed-attempt-11485635` cut from M56a; Draft PR; STOP before Ready/merge. **Consequences.** The ONE authorized submission is SPENT (AC5): no retry, resubmission, continuation, completion job, or repair. **No v1.22 addendum is possible from this attempt** — allowlist C's v1.22 carries measured thread-pinning values from a successful matrix (v1.5 owed-numbers; v1.8 item 5) and this attempt produced zero timing artifacts; the number is NOT burned and stays reserved. **A7 remains OWED.** The Della worktree and its captured `runs/d19_a7_timing` are preserved untouched. Next acts, each a separate author authorization, none begun: (1) strictly read-only Della diagnostic to attribute `environment` line 49; (2) bounded D56b correction of the submit script's environment-setup region — an execution-byte change that breaks amended-§7 item 1 by construction, hence its own review cycle and a NEW launch anchor; (3) a fresh byte-exact launch authorization.

**D56 Update 6 — read-only Della diagnostic attributed `environment: line 49` to the anaconda3/2024.6 modulefile's unguarded `${PS1}` read; D56b bounded correction implemented (`export PS1="${PS1-}"` plus attempt-2 worktree `_02`); reviewed head B and launch anchor M56b defined; hermetic, NO Della operation, NO benchmark (2026-07-22).** **Diagnostic, observed facts (separately authorized, strictly read-only, performed on Della 2026-07-22 before D56b began; recorded here).** In a disposable noninteractive child shell with `PS1` unset and `set -u` enabled: `module purge` passes, and `module load anaconda3/2024.6` reproduces the byte-identical 44-byte failure line `environment: line 49: PS1: unbound variable`. The anaconda3/2024.6 modulefile emits 51 shell lines; emitted line 50 carries the first unguarded read, `export _LOCAL_OLD_PS1="${PS1}"`. The emitted text executes through the environment-imported `_module_raw` wrapper, which explains both the `environment` source label and the one-line offset (reported line 49 against emitted-load-text line 50). No project import, data load, benchmark, or scientific computation occurred during the diagnostic. This converts Update 5's bounded lines-29-33 bracket from inference to identification and attributes the previously unidentified `environment` source. **Exact reproduction and prevention proof.** Defining and exporting an empty `PS1` inside the disposable child before `set -u` makes `module purge`, `module load anaconda3/2024.6`, conda-hook generation and evaluation, and activation of `/home/sc8918/.conda/envs/bistar_gp` all pass. **Chosen correction (frozen disposition).** Exactly `export PS1="${PS1-}"`, placed after `set -euo pipefail` and before the first module operation. Nounset stays enabled for the entire script; no `set +u` bracketing of the module/conda region, because bracketing would weaken strictness across that whole region instead of defining the single variable the modulefile reads. **Attempt-2 path.** Attempt 1's Della worktree `/scratch/gpfs/SUYOGHC/bistar_gp_a7_exec` remains preserved and registered, never removed, reset, cleaned, or reused; protocol P2 now states its presence is not a collision for attempt 2. Attempt 2 uses exactly `/scratch/gpfs/SUYOGHC/bistar_gp_a7_exec_02` in every live binding: Slurm `--chdir`, script `EXEC_ROOT`, protocol P2 collision guard and worktree-add, P3 explicit worktree path, P7 scp source, and the test expectations. The in-worktree evidence path `runs/d19_a7_timing/` and the local staging path `runs/d19_a7_timing_incoming` (currently absent) are unchanged. **Implementation (D56b, six-file allowlist).** Implementer: Codex **gpt-5.6-sol** xHigh (identity verified from the session banner AND the rollout-log `"model"` field, session `019f8814`), with two orchestrator refinements before review, both re-verified: the nounset guard regex unanchored so a mid-line `set +u` cannot evade it, plus a `set +o` denial; and the two §7 definition sentences moved to the D56a `=` convention. Changed files bounded by the six-file allowlist: `experiments/submit_d19_a7_bench.slurm` (three hunks: `--chdir` to `_02`, `EXEC_ROOT` to `_02`, and the comment plus `export PS1="${PS1-}"` inserted between `set -euo pipefail` and `fail()`), `tests/test_d19_a7_protocol.py`, `docs/d19-a7-execution-protocol.md`, and DECISIONS plus SCRATCHPAD at the implementation head; `Notes/CHATLOG.md` follows in the Notes-only session tail. Nine new discriminating hermetic tests (10 items): literal-once-and-position between strict mode and `module purge`; a disposable `/bin/bash` reproduction with a fake modulefile tail whose final line is exactly the unguarded read, passing with the correction in place and dying with `PS1: unbound variable` on stderr when the correction is absent OR moved after module load; nounset never relaxed (no `set +u` cluster and no `set +o` anywhere in the script); every live attempt-2 binding on `_02` (prefix-count equality proves the bare attempt-1 root never appears in the script); the attempt-1 path present in the protocol only outside fenced command blocks (history and preservation text, never a live target); the four failed-attempt evidence blobs pinned by sha256 AND byte size; single-submission STOP-only semantics (exactly one `sbatch` invocation, `P5 — Submit once.`, `no retry`, fresh byte-exact authorization language); and the D56b amendment recorded (D56b, M56b, the failed-attempt merge SHA, the correction literal, the `_02` root). **Topology (protocol §7, D56b amendment, appended without rewriting D56/D56a history).** `B` = the reviewed D56b code head, the last non-Notes commit of the PR (a name unrelated to §8's evidence-commit allowlist B); `M56b` = the true-merge launch anchor of the D56b PR (second parent = the PR head); D56b deliberately does not name its own future merge SHA; every `<M56>` placeholder in §4/§5 reads as `<M56b>` for attempt 2. Launch closure requires ALL of: submit script, protocol document, and protocol test file byte-identical across `B..M56b` (reviewed-surface closure; any post-`B` commit before merge must be Notes-only and explicitly identified in the launch authorization); validator, vehicle, and `bistar_gp/` byte-identical across `H'..M56b` (H' unchanged, `4c9b79ae8fbe42ceeacbeac1f99a2cc1599ece7a`); the complete `7d234e9f..M56b` name-diff limited to exactly the six-file D56b allowlist (pinning the attempt-1 evidence blobs and the remainder of the tree, the D56a-item-6 pattern); `origin/main` equal to `M56b` with true-merge second-parent topology; a fresh byte-exact author authorization naming `M56b` (the spent attempt-1 authorization does not carry over); and script/validator/V0 enforcement bindings re-anchored at `M56b`. **Verification.** Focused `tests/test_d19_a7_protocol.py`: **48 passed** (38 prior + 10 new items). Frozen guard `tests/test_slurm_argparse.py`: **12 passed** (file untouched; it re-parses the changed script). Frozen firewall `tests/test_d19_bench_firewall.py`: **68 passed** (untouched). Full `python -m pytest -q`: **1167 passed, 2 skipped, 0 failed** (0:04:42), against the same-session baseline **1157 passed, 2 skipped** measured at the base `7d234e9f` before implementation; the delta reconciles exactly as the 10 new test items. Changed-file proof: `git diff --name-only` from `7d234e9f` stays within the six-file allowlist at every head (five files at the implementation head; CHATLOG joins in the Notes-only tail; re-proven at the final head). Attempt-1 evidence re-proof: the four blobs re-hashed byte-identical to the Update 5 pins, now also enforced by a standing test. Absence re-proof: local `runs/d19_a7_timing/`, staging `runs/d19_a7_timing_incoming`, v1.22, and the burned v1.18 all absent. Base-state proof recorded before implementation: `git diff H'..7d234e9f -- experiments/d19_a7_validate.py experiments/d19_bench.py bistar_gp/` is empty. **Provenance.** Hermetic throughout: local file edits, disposable local shells, and source/text assertions only. NO Della operation, module or conda probe, worktree creation, submission, retry, benchmark, Mauna load, holdout access, v1.22 addendum, or scientific computation occurred in D56b. The one authorized attempt-1 submission remains SPENT; A7 remains OWED.

**D56 Update 7 — two-model read-only review gate on D56b: Codex gpt-5.6-sol xHigh REVISE, Opus 4.8 APPROVE; every finding independently verified; the SINGLE bounded correction pass applied and mutation-verified; reviewed head B = `e4894e2e358f2b4db7a9bed315c315a5e6f6c7ec` (2026-07-22).** Reviewer 1: FRESH Codex **gpt-5.6-sol** xHigh, sandbox read-only, at implementation head `08e7d2d798934d74f3a47763f08e854c056bef8a`; identity verified from the session banner AND the rollout-log `"model"` field (session `019f8821`); its final-message self-identification read "GPT-5" and is superseded by the configured-session verification (the D55 precedent separating self-report from session identity). Verdict **REVISE**: 1 BLOCKER, 4 MAJOR, 2 MINOR; its frozen-surface drift check was clean. Reviewer 2: FRESH **Opus 4.8** (model id `claude-opus-4-8[1m]`, transcribed from its own context), strictly read-only instructions, same head. Verdict **APPROVE** with 1 MINOR (identical to Codex MINOR 6) and 4 NOTEs; it independently confirmed the correction's bash semantics, the discriminating-test matrix, hermeticity and bash-3.2 portability (its own focused run: 48 passed), closure satisfiability (the `H'..7d234e9f` execution-byte diff is empty), and zero frozen-surface drift. Orchestrator cross-verification and dispositions: (1) BLOCKER CONFIRMED — the closure pinned only the submit script across `B..M56b` while item 3 permits allowlist-file changes, so a post-review commit or merge resolution could weaken the protocol or tests unseen; CLOSED by widening item 1 to reviewed-surface closure (script + protocol document + protocol test file byte-identical across `B..M56b`, post-`B` commits Notes-only and explicitly identified in the launch authorization), with T9 now pinning both fenced closure commands and the load-bearing phrases. (2) MAJOR CONFIRMED — a commented-out `# export PS1="${PS1-}"` kept the count-and-order check green while the reproduction inserted the constant independently; CLOSED by line-exact assertions (every line containing the literal must equal it) and by extracting the correction line from the script itself into the reproduction driver. (3) MAJOR CONFIRMED — the P2 STOP target and P3 change-directory prose bindings were unguarded, and a P2 revert would permanently block launch because the attempt-1 worktree must remain present; CLOSED by positive prose assertions on both bindings plus a per-line rule that every bare attempt-1 occurrence carries preserved/attempt-1 context. (4) MAJOR CONFIRMED — `set +e +u` evaded the regex; CLOSED by a token-level `set` argument parser denying any `+` cluster containing `u` and any `+o`, kept alongside the regex and the `set +o` denial. (5) MAJOR PARTIALLY CONFIRMED — textual presence checks cannot bind semantics against a reviewed adversarial rewrite; the structural closure is disposition (1)'s reviewed-surface byte-pin, and the accidental-drift surface was additionally hardened by pinning the document-wide `sbatch` count to exactly 2 and §6's verbatim no-retry sentence. (6) MINOR CONFIRMED, shared with Opus — Update 6's "exactly six"/completed-proof wording was premature at the implementation head; reworded to five-at-head plus the Notes-only CHATLOG tail, bounded by the allowlist and re-proven at the final head. (7) MINOR CONFIRMED as ambiguity — the anchor name `B` collides with §8's "evidence commit allowlist B"; CLOSED by an explicit disambiguation sentence (Git commit anchor versus file list; the name `B` itself is task-fixed). Opus's remaining NOTE (the §2 present-tense attempt-1 worktree sentence) was ADJUDICATED NO ACTION: dated 2026-07-21 reconnaissance narrative; editing it would rewrite D56 history, and §4 plus the D56b amendment carry the live bindings. Mutation verification of the strengthened guards: five mutants, each killed by the intended tests with byte-identical restores and a green control — commented-out correction (3 failed), `set +e +u` appended (1 failed), bare P2 STOP target (1 failed), added retry-sbatch sentence (1 failed), deleted closure command (1 failed); control 48 passed. The single authorized correction pass was implemented by the orchestrator per the adjudication (the D56 Update 3 precedent), delta exactly `tests/test_d19_a7_protocol.py` + `docs/d19-a7-execution-protocol.md` + `Notes/DECISIONS.md`, commit **B = `e4894e2e358f2b4db7a9bed315c315a5e6f6c7ec`**, the last non-Notes commit of the PR. Post-pass verification: focused 48 + 12 + 68 = **128 passed**; full `python -m pytest -q` **1167 passed, 2 skipped, 0 failed** (unchanged item count; strengthened assertions only). This Update, the SCRATCHPAD alignment, and the CHATLOG entry form the Notes-only tail after `B`, explicitly identified here per the reviewed-surface closure rule. These are AI reviews, not GitHub reviews or CI.

**D56 Update 8 — exact-head confirmation round at B = `e4894e2e358f2b4db7a9bed315c315a5e6f6c7ec`: SPLIT verdict (fresh Codex FINDINGS, fresh Opus APPROVE — not unanimous); the backslash-continuation residual mechanically confirmed and ACCEPTED by author adjudication as a known, non-launch-blocking guard limitation; Update 7's class-wide CLOSED wording for finding (4) superseded (2026-07-22).** Both Update 7 reviews ran at implementation head `08e7d2d798934d74f3a47763f08e854c056bef8a`; neither inspected `e4894e2e`. A separately authorized, strictly read-only exact-head round therefore ran 2026-07-22, each reviewer FRESH and seeded with its original findings VERBATIM (recovered byte-exact from the Codex `019f8821` rollout and the original Opus subagent transcript). Reviewer 1: Codex **gpt-5.6-sol** xHigh (session `019f8998`; banner AND rollout-log fields verified `gpt-5.6-sol`/`xhigh`; sandbox read-only; its final-message "GPT-5" self-identification again superseded by the configured-session identity, the D55 precedent). Verdict **FINDINGS**: closures 1, 2, 3, 6, 7 CLOSED; (4) NOT-CLOSED because valid Bash `set +e \` plus newline plus `+u` joins by continuation into `set +e +u`, disabling nounset while evading the retained regex, the per-physical-line `_set_option_args` token parser, and the `set +o` check; (5) NOT-CLOSED because a `sbatch`-free retry sentence keeps the STOP-only test green; new-defect section NONE; frozen surfaces clean. Reviewer 2: **Opus 4.8** (`claude-opus-4-8` from the transcript model field, self-transcribed `claude-opus-4-8[1m]`; all 18 SHA-bearing git commands used the correct full SHA). Verdict **APPROVE**: all seven dispositions VERIFIED-CLOSED, its own MINOR RESOLVED, the §2 adjudication ACCEPTED, no new defects, zero frozen-surface drift, focused suite 48 passed on the target bytes. **Orchestrator adjudication of the split.** Finding (4) was reproduced by construction: the guard logic copied verbatim from B and run in memory passes the real script (control), passes the continuation mutant (all three guards evaded), and catches single-line `set +e +u`; a disposable `/bin/bash` run shows the shell flags dropping from `ehuBc` to `hBc` after the continued cluster with a bare unbound expansion surviving (exit 0) while the control aborts on `unbound variable` (exit 1); the frozen `tests/test_slurm_argparse.py` guard parses only sbatch invocation flags and offers no backstop. Codex is CORRECT on (4); Opus's VERIFIED-CLOSED grade covered only the named single-line forms. Finding (5) is textually true but restates Update 7's recorded partial disposition (textual checks are accidental-drift hardening; the item-1 byte-pin is the authoritative protection), so that disposition is retained. **Author adjudication (cast 2026-07-22).** The residual is ACCEPTED as a known, NON-LAUNCH-BLOCKING limitation of the test guard; NO further code or test correction pass is authorized. Rationale: (i) the exact attempt-2 script at B contains `set -euo pipefail`, contains no nounset-relaxing command, and retains nounset throughout its actual execution path; (ii) reviewed-surface closure requires the submit script, protocol document, and protocol test file byte-identical across `B..M56b`, so the demonstrated mutant cannot enter attempt 2 without violating the launch gate; (iii) `_set_option_args` is a bounded source guard, not a complete parser of Bash logical commands; (iv) the limitation matters for a future amendment cycle, where any changed script receives fresh review, and does not change the safety or identity of the exact attempt-2 launch bytes; (v) finding (5) retains the already-recorded disposition. **Supersession.** Update 7's statement that disposition (4) closed the whole class is RETRACTED; the accurate statement is that the named single-line forms (for example `set +e +u`) are closed while backslash-continued option clusters remain a known, author-accepted guard limitation. **Standing.** `B = e4894e2e358f2b4db7a9bed315c315a5e6f6c7ec` is the exact-head-reviewed D56b code anchor, carrying this explicit author-accepted residual; the confirmation round is recorded as SPLIT and must not be described as unanimous, CONFIRMED-CLOSED, or APPROVE by both models. The Notes-only tail after B now comprises exactly two commits, `7d077aff74b6cc2aabd11411dc9f134671afbf77` (Update 7) and this Update 8 commit, both explicitly identified here per the reviewed-surface closure rule. No non-Notes byte changed; the suite was NOT rerun (1167 passed / 2 skipped / 0 failed already applies to B's tree; this tail is Notes-only). These are AI reviews, not GitHub reviews or CI.

**D56 Update 9 — D56c (lightweight): A7 attempt 2 (job 11497561) FAILED pre-cell on a missing `arviz` dependency; Della env re-frozen (prereg v1.22, +11 packages); `_03` repath + §7 D56c amendment; attempt 2 folded into Notes with no separate evidence PR (2026-07-22).** After D56b merged (M56b `d9c924fc35cc771775732cb431014a25de8a6400`), the fresh authorization naming M56b was cast and attempt 2 user-executed: P0–P5 clean (execution-time topology PASS, detached `_02` worktree at M56b = 2107 files pristine, exactly one `sbatch` → job **11497561**), then **FAILED in 37 s, exit `82:0`**, on della-i13n7 (inside the frozen 90-node pool). sacct parent row: `11497561|FAILED|82:0|00:00:37|02:00:00|00:03.748||della-i13n7|2026-07-22T10:34:45|2026-07-22T10:34:50|2026-07-22T10:35:27`. stdout 0 B (no ENV-OK/CELL/MATRIX); stderr 475 B (sha256 `6eb2797defe136e57385d9efe9bcfd47ecfffcf5ad36026808217b5be20267b8`): `ModuleNotFoundError: No module named 'arviz'` at `bistar_gp/__init__.py:36` → `mcse_strategy.py:16`, then `PREFLIGHT-FAIL(82)`. The submit-script preflight `import bistar_gp` (script line 80) caught it and failed closed before cell 1 — no scientific computation, no Mauna load, no artifact. The one authorized attempt-2 submission is SPENT (AC5). **Root cause.** The env-freeze / D55 key-inventory audit pinned only five package versions and never test-imported `bistar_gp` on Della, so its full import closure (arviz + transitive jsonschema/referencing) was never verified present; the local suite passes because the Mac env has them. **Remedy (author elected the lightweight path after a proportionality assessment; the heavy pieces — a separate failure-evidence PR ceremony and a runtime full-manifest exact-match gate — were skipped).** The frozen `bistar_gp` env at `/home/sc8918/.conda/envs/bistar_gp` was extended by installing `arviz`+`jsonschema` with numpy/scipy/pandas/matplotlib/torch/gpytorch/pyro-ppl held by constraint — a pure addition of 11 packages, zero removals, zero version changes (proven by `comm` on before/after `pip list --format=freeze`); the five pins stay python 3.11.14 / torch 2.10.0+cu128 / gpytorch 1.15.1 / pyro 1.9.1 / numpy 2.4.2; the preflight then printed `ENV-OK`. **D56c change (branch `fix/d19-a7-d56c-env` off M56b, eight-file allowlist).** `_03` repath in the submit script (`--chdir`/`EXEC_ROOT`), protocol (P2/P3/P7 + §7 D56c amendment defining `M56c`), and tests; the committed 69-package manifest `docs/d19_a7_freeze/bistar_env_after.txt` (1386 B, sha256 `d832d426ec5a83e3f1da3275c289323c8732f2644038efb15b2eb1567b085aa1`); prereg addendum **v1.22** recording the re-freeze (versioning reassignment: the successful measured-results addendum moves to **v1.23**); a preparation-time read-only `import bistar_gp` on Della added to the launch checklist; the attempt-1 (bare) and `_02` worktrees preserved as history. The attempt-2 failure evidence stays preserved in the `_02` worktree on Della and is recorded here rather than in a committed evidence directory. **Verification.** Focused `tests/test_d19_a7_protocol.py` **49 passed** (48 + the new D56c amendment test); frozen `tests/test_slurm_argparse.py` **12** + `tests/test_d19_bench_firewall.py` **68**; full `python -m pytest -q` **1168 passed, 2 skipped, 0 failed**. **Provenance.** Hermetic repo edits plus one author-executed constrained env install on della9; no benchmark, submission, Mauna load, or scientific computation. Attempt 2 SPENT; A7 remains OWED.

**D56 Update 9 — hardening pass (2026-07-22, pre-review, per an external Codex hardening pass; lightweight disposition preserved — NO runtime full-manifest gate, NO attempt-2 evidence PR).** A second D56c commit tightened the amendment and tests without new runtime machinery: (1) **preparation-time environment enforcement** — the read-only Della check before a future authorization is cast now requires BOTH exact byte-for-byte equality of the live `pip list --format=freeze` against the committed 69-package manifest AND `import bistar_gp` under the five pins, with the honest disclosure that the complete inventory is enforced only at preparation time (the submit job still enforces the five pins + `import bistar_gp`, exit 82, NOT the manifest) and the prep-to-submission interval is a disclosed user-controlled trust interval; (2) **explicit reviewed-surface closure** — the vague "D56b rule applies in shape" was replaced by an explicit D56c rule naming the reviewed head **R56c** (a new unambiguous anchor) and pinning five files byte-identical `R56c..M56c` (submit script, protocol tests, protocol document, prereg addenda, committed manifest), post-`R56c` commits Notes-only and identified, the eight-file total-surface allowlist and true-merge topology retained, the `H'` execution-byte closure retained; (3) **new hermetic tests** pin the manifest's sha256 (`d832d426…`) / size (1386) / 69-line count, the five-file reviewed-surface closure command verbatim, and the BOTH-check preparation enforcement (omitting either the inventory-equality or the import check trips a test); (4) **wording corrections** — "no benchmark JSON artifact resulted", `bistar_env_before.txt` retained uncommitted in Della home, and the complete inventory is not described as runtime-enforced. Verification: focused `tests/test_d19_a7_protocol.py` **50 passed** (49 + the manifest-integrity test); frozen `tests/test_slurm_argparse.py` **12** + `tests/test_d19_bench_firewall.py` **68**; full `python -m pytest -q` **1169 passed, 2 skipped, 0 failed**. Cumulative `M56b..HEAD` diff stays within the eight-file allowlist. A bounded two-model exact-head review round (fresh Codex gpt-5.6-sol xHigh + fresh Opus 4.8, read-only) follows on the corrected head; findings are returned for author decision, not auto-corrected.

**D56 Update 10 — D56c two-model exact-head review round (Codex FINDINGS, Opus APPROVE), single bounded correction closing all three verified findings, both fixes CONFIRMED-CLOSED; reviewed head R56c = `0b10da9d48d9c6127f5c8634fd12bc16417aad53`; this is the identified Notes-only tail after it (2026-07-22).** The bounded review ran at the hardening head `491680d` (a first launch of both reviewers stalled ~4 h on a machine-sleep gap and was relaunched fresh). Reviewer 1: fresh Codex **gpt-5.6-sol** xHigh (session `019f8c44`, banner + rollout model/effort verified, read-only sandbox) verdict **FINDINGS** — two test-discrimination points: (F1) the D56c amendment test asserted the two preparation phrases independently but not their BOTH/AND conjunction; (F2) no test read the prereg or asserted v1.22, only v1.23 was pinned. Reviewer 2: fresh **Opus 4.8** (transcript model field `claude-opus-4-8`) verdict **APPROVE** with one MINOR — (F3) §8's post-run-label wording ("allowlist C is … v1.22 … latest addendum is v1.21") was stale after the reassignment though superseded in-file by the §7 D56c amendment. Every substantive dimension (environment enforcement + honest scope, `_03` repath, launch-closure coherence/satisfiability, v1.22 authority, provenance wording, frozen-surface drift) verdicted OK by both. Orchestrator independently confirmed all three findings against source; none launch-blocking (each mitigated by the reviewed-surface byte-pin closure), all three defense-in-depth. Author chose option (b): one bounded correction commit **`0b10da9d48d9c6127f5c8634fd12bc16417aad53`** (no amend; two files — tests + protocol — within the eight-file allowlist) closed all three: F1 the test now asserts `must require **BOTH**` plus the AND-joined `and (b) a successful \`import bistar_gp\``; F2 a new `test_prereg_records_the_v122_env_refreeze` reads the prereg and asserts the v1.22 addendum + manifest path + full sha256 + the "measured-results addendum becomes **v1.23**" reassignment; F3 §8 gained a concise supersession pointer with the historical numbering left unrewritten. Orchestrator mutation-verified in-memory (F1 BOTH→EITHER / and→or → fail; F2 v1.22-relabel and reassignment-removed → fail; controls pass). Focused **51 passed** (50 + the prereg test); frozen 12 + 68; full `python -m pytest -q` **1170 passed, 2 skipped, 0 failed**. Focused per-reviewer confirmation at `0b10da9`: Codex gpt-5.6-sol xHigh (session `019f8c63`, banner + rollout verified) **CONFIRMED-CLOSED** F1+F2 by re-attempting the exact mutations; Opus 4.8 (`claude-opus-4-8`) **CONFIRMED-CLOSED** F3; neither found a new defect. The reviewed D56c code head is now **R56c = `0b10da9d48d9c6127f5c8634fd12bc16417aad53`** (the correction commit is the last non-Notes commit). This Update, the SCRATCHPAD alignment, and the CHATLOG entry are the identified Notes-only tail after R56c, per the §7 D56c reviewed-surface closure rule (post-`R56c` commits must be Notes-only and explicitly identified). These are AI reviews, not GitHub reviews or CI. A mechanical Ready-closure authorization is returned; STOP before Ready.

**Status.** D56 merged (PR #24, M56 `66ca91cd8a5e0a2bbb7d984b2e5298707160d6c0`); D56a merged (PR #25, M56a `e9457d719bfbaea9e890329687b8c166b872570a`); attempt-1 failure evidence merged (PR #26, `7d234e9ffad6b154e7523507658a6999e7bb6c53`); D56b merged (PR #27, launch anchor M56b `d9c924fc35cc771775732cb431014a25de8a6400`). The attempt-1 authorization is SPENT (job 11485635, Update 5); the attempt-2 authorization is SPENT (job 11497561, arviz pre-cell failure, Update 9). The Della `bistar_gp` env is re-frozen (prereg v1.22, +11 packages, 69-package manifest committed). **D56c is implemented AND two-model exact-head reviewed on branch `fix/d19-a7-d56c-env` off M56b (eight-file allowlist; `_03` repath + §7 D56c amendment with explicit reviewed head `R56c` + committed env manifest + prereg v1.22; hardening + review-correction applied). Reviewed head R56c = `0b10da9d48d9c6127f5c8634fd12bc16417aad53`; all three review findings closed and both fixes CONFIRMED-CLOSED (Update 10); the Notes-only tail after R56c is identified. Draft PR #28, STOP before Ready; a mechanical Ready-closure authorization is returned.** **HARD STOP before: Ready / merge / any Della contact or worktree creation / any submission or retry / benchmark / data load / Mauna access / v1.23 measured-results addendum / M2d / science.** Attempt 3 additionally requires `origin/main == M56c` (the future D56c true merge), the preparation-time read-only Della check (exact 69-package inventory equality AND `import bistar_gp`), and a NEW byte-exact author authorization naming `M56c`. A7 remains OWED. D45 remains permanently UNVALIDATED_ATTEMPT; v1.18 remains absent; D53 PRESERVE_STOP is unaltered. Deletion of the two superseded pre-D22 anchors remains an OPEN separate author decision.

**D56 Update 11 — D56d post-run, outcome-informed correction to the A7 validator's free-text `condition` semantics plus explicit V0 validator anchoring; implementation complete, review gate pending (2026-07-23).** M56c is `725e5f194de7bda12475f0d2a64893aa5cf5315f`. A7 job **11517022 COMPLETED successfully**; recovery R1–R10 passed; the recovered twelve-entry bundle `runs/d19_a7_timing_recovery_11517022_incoming` is preserved unchanged. On that bundle, V0–V4, V6–V10, and V12–V15 passed, but the ORIGINAL validator returned **14/16 PASS**: V5 and V11 alone failed because raw-substring `condition` matched `conditional` in the original `lscpu` Spectre-v2 host-metadata line (`IBPB conditional`). This amendment is explicitly **post-run and outcome-informed**. It is NOT preregistered and is NOT an original clean validation; no timing magnitude has been interpreted; the run remains UNVALIDATED until an amended, merged validator passes.

**Decision and bounded implementation.** In validator free-text scanning only, `condition` now uses `condition(?!al)`: the unrelated `conditional` family passes, while `condition`, separators/suffixes, inflections, and prefixed occurrences remain rejected. The finding text is unchanged. The vehicle's `FORBIDDEN_KEY_SUBSTRINGS` table still contains `condition`; forbidden-KEY checks, the closed-world artifact schema, `_artifact_scan_text`, and every other token's semantics are unchanged. V0 adds optional lowercase-40-hex `--validator-sha`: the vehicle blob is independently bound to `--expected-sha`, the validator blob to `--validator-sha`, and omission defaults the latter to the former so existing invocations behave identically. Discriminating unit/integration tests cover the exact pass/fail matrix, the real host line, other substring and whole-word semantics, argument rejection, independent vehicle/validator mismatches, and the default binding. Verification: `tests/test_d19_a7_protocol.py` **74 passed**; `tests/test_slurm_argparse.py` + `tests/test_d19_bench_firewall.py` **80 passed**. No full suite was run.

**D56d closure and standing.** `R56d` is the future reviewed D56d code head (last non-Notes commit off M56c); `M56d` is the future true-merge anchor whose second parent is the PR head. Require the three-file reviewed surface (`experiments/d19_a7_validate.py`, protocol tests, protocol document) byte-identical `R56d..M56d`, every post-`R56d` commit Notes-only and identified, empty `git diff H'..M56d -- experiments/d19_bench.py bistar_gp/` with H′ = `4c9b79ae8fbe42ceeacbeac1f99a2cc1599ece7a`, the complete M56c..M56d name-diff limited to the six-file D56d allowlist, and `origin/main == M56d`. The validator is intentionally absent from the H′ execution-byte diff because D56d amends it; its reviewed M56d identity is enforced through V0 `--validator-sha`. A fresh explicit authorization is required before the exact post-merge revalidation command recorded in the protocol; expected result is all V0–V15 PASS. **PENDING GATE:** fresh read-only Codex gpt-5.6-sol xHigh plus fresh Opus 4.8 exact-head review, AI reviews rather than GitHub reviews or CI. No commit, push, PR, merge, evidence mutation, post-merge revalidation, or results addendum is performed here.

**D56 Update 12 — D56d two-model exact-head review round (Codex FINDINGS, Opus APPROVE), single bounded correction pass closing both verified test-discrimination findings, mutation-verified; reviewed head R56d = `e6f45bc07169691bf9cb55ae4c96dabb2a67e3eb`; this is the identified Notes-only tail after it (2026-07-23).** The bounded review ran at the D56d implementation head `60805195ba0da58dd17b0e6b0c4599e81fb226fd`. Reviewer 1: fresh Codex **gpt-5.6-sol** xHigh (session `019f8d40`, banner + rollout model/effort fields verified `gpt-5.6-sol`/`xhigh`, read-only sandbox) verdict **FINDINGS** — two MINOR test-discrimination points, both in `tests/test_d19_a7_protocol.py`: (F1) the malformed-`--validator-sha` parametrization omitted valid-lowercase-hex-but-wrong-length cases (39/41 char), so a `[0-9a-f]{40}`->`[0-9a-f]+` length-check regression would stay green; (F2) no test drove a VALID `--validator-sha` distinct from `--expected-sha` end-to-end through `run()` — the `_v0` tests inject `validator_sha` straight into the context dict and the `run()`-level tests cover only the malformed and omitted paths, so a `run()` that ignored `args.validator_sha` (rebinding the validator blob to `expected_sha`) would stay green. Reviewer 2: fresh **Opus 4.8** (transcript model field `claude-opus-4-8`) verdict **APPROVE / NONE** — matcher semantics, V0 decoupling, discriminating tests, amendment truthfulness, and the frozen-surface check all verified clean; one explicitly below-bar precision NOTE that §7's "For validator free-text scans only" under-scopes because `_scan_forbidden_text` also feeds V5's artifact-JSON-text scan (an under-claim that weakens no gate; not actioned, surfaced to the author). Orchestrator independently confirmed both Codex findings against source; neither is launch-blocking (the implementation matcher and the V0 split are correct — these are test-discrimination gaps only). **Correction pass (pre-authorized single bounded pass; orchestrator-applied per design line 59 "at most ONE bounded correction pass; then re-confirm", the D56d cast deliberately electing correction-then-reconfirm rather than D56c's return-for-decision).** Purely additive, test-only, within the six-file allowlist, no reviewed code byte touched: (F1) a new `test_wrong_length_hex_sha_rejected_at_parse` parametrized over `"f"*39`/`"f"*41` asserts the specific `--expected-sha`/`--validator-sha` "must be 40 lowercase hexadecimal characters" argument-failure message IS emitted (message-presence, deliberately NOT the existing two-output `.replace` comparison — under the `+` mutant those inputs stop short-circuiting and the two evidence-processing outputs can compare equal, leaving that structure green); (F2) a new `test_valid_validator_sha_flows_through_run` monkeypatches `_dependency_blob_mismatches` to capture its arguments and asserts a valid `--validator-sha "b"*40` (!= `--expected-sha`) reaches it as `(EXPECTED_SHA, "b"*40)` end-to-end through `run()`. Orchestrator mutation-verified BOTH against the actual validator with byte-identical `git checkout` restores and a green control: `{40}`->`+` fails F1 (both parametrized cases); `run()` rebind `validator_sha = args.expected_sha` fails F2. The validator (`experiments/d19_a7_validate.py`) and protocol document are byte-identical to `60805195` (`git diff 60805195..R56d -- experiments/d19_a7_validate.py docs/d19-a7-execution-protocol.md` empty); the correction changes only `tests/test_d19_a7_protocol.py` (+48 lines, +3 test instances). Focused `tests/test_d19_a7_protocol.py` **77 passed** (74 + 3); frozen `tests/test_slurm_argparse.py` **12** + `tests/test_d19_bench_firewall.py` **68**; full `python -m pytest -q` **1196 passed, 2 skipped, 0 failed** (0:07:34). **Closures re-proven at R56d.** `60805195..R56d` name-diff is exactly `tests/test_d19_a7_protocol.py`; complete `M56c..R56d` name-diff is exactly the six-file allowlist (validator, protocol tests, protocol document, DECISIONS/SCRATCHPAD/CHATLOG); frozen surfaces `git diff M56c..R56d -- experiments/d19_bench.py bistar_gp/ docs/prereg-addenda-d19.md docs/d19_a7_freeze/ tests/test_d19_bench_firewall.py tests/test_slurm_argparse.py runs/` empty; H' execution-byte closure `git diff 4c9b79ae..R56d -- experiments/d19_bench.py bistar_gp/` empty. The reviewed D56d code head is now **R56d = `e6f45bc07169691bf9cb55ae4c96dabb2a67e3eb`** (the correction commit is the last non-Notes commit; the two-model review ran at `60805195` and the single bounded correction produced R56d — the D56c Update 10 precedent). This Update, the SCRATCHPAD alignment, and the CHATLOG entry are the identified Notes-only tail after R56d, per the §7 D56d reviewed-surface closure rule (post-`R56d` commits Notes-only and explicitly identified). These are AI reviews, not GitHub reviews or CI. **Standing.** The corrected head R56d has NOT itself been re-reviewed by both models (the review ran at `60805195`; the correction is test-only, additive, and mutation-verified). Draft PR #29 remains STOPPED before Ready; a fresh exact-head re-review of R56d before Ready is available at author option. HARD STOP before: Ready / merge / M56d / any Della contact or worktree creation / any submission, retry, benchmark, or Mauna load / the post-merge revalidation command / the recovery-labeled evidence commit / the truthful Notes recovery record / the v1.23 measured-results addendum. A7 remains OWED.

**D56 Update 13 — A7 job-11517022 evidence-and-Notes closure: validated timing evidence placed at the canonical path, original incomplete capture preserved, recovery record added; E57/N57 topology; Draft PR, STOP before Ready (2026-07-23).** Executed under pinned plan v3 (`/Users/sc8918/a7_evidence_closure_plan.md`, 17976 B, sha256 `1f19d14e0c9915d28bd78ab3a83f25250201524d144551fd8d7c20370fa82d30`) on branch `evidence/d19-a7-timing-11517022` off origin/main M56d. Anchors: execution **M56c** `725e5f194de7bda12475f0d2a64893aa5cf5315f`, validator **M56d** `5fcc2d316f6894ed793418d9ab16274e6c8a1ad2`. **Actions.** Freeze-verified all 23 existing evidence files against the pinned SHA-256/size table; two guarded no-clobber `mv` moves placed the validated recovered 12-entry bundle at the canonical `runs/d19_a7_timing/` (§8 allowlist B) and preserved the original incomplete 11-entry capture at `runs/d19_a7_timing_original_incomplete_11517022/`; added `runs/d19_a7_timing_recovery_record_11517022.md`. The canonical directory revalidated **16/16 (V0–V15 PASS, exit 0)** against `--expected-sha M56c --validator-sha M56d`. Both `slurm-11517022.out` files were force-added (`.gitignore:29 slurm-*.out`); `git ls-files` == 12 (canonical) and == 11 (original). **Provenance facts (no timing magnitude interpreted).** The 10 shared payload files are byte-identical across the two bundles; the recovered 11-line manifest (`8c27e40b…`, 985 B) = the original 10-line manifest (`a3aadf32…`, 902 B) plus the single `job_metadata.txt` line (delayed post-run P6 `sacct` recovery; the original P6 redirect was CWD-relative and never wrote `job_metadata.txt`). Job `11517022` COMPLETED 0:0 on della-i13n2 (inside the frozen pool); ENV-OK on the five pins; eight CELL exits + MATRIX-COMPLETE. **Topology.** **E57** = the evidence + recovery-record commit `7d0b8e57ed60c863f24efb97dfd6ad7e1c8e9455` (24 files; `M56d..E57` is exactly those 24 and nothing else). **N57** = this Notes-only tail (DECISIONS Update 13 + SCRATCHPAD alignment + CHATLOG entry); `E57..N57` contains only the three Notes files; `M56d..N57` is the complete 27-file PR diff. The 23 evidence-bundle files and both manifests are immutable from E57 onward. **Review gate (pending).** One fresh Codex gpt-5.6-sol xHigh strictly read-only reviewer at exact final head N57 over the complete `M56d..N57` diff (including Notes), verifying every evidence/record byte at N57 equals E57; the reviewer does NOT rerun the validator and does NOT interpret timing magnitudes; the Opus 4.8 orchestrator independently verifies every finding against source; GLM 5.2 only on material Codex↔orchestrator disagreement; no redundant fresh Opus reviewer. At most one bounded non-evidence correction pass (recovery record / Notes / review metadata only) producing **C57/F57** and a re-confirmed `M56d..F57`; any finding requiring a 12/11-file bundle or manifest change STOPs and returns to the author (evidence bytes and manifests immutable). **Standing.** Della source and the spent worktrees `…_exec` / `…_exec_02` / `…_exec_03` are untouched; no benchmark, Della action, deletion, or v1.23. Draft PR opened; **STOP before Ready**. The v1.23 measured-results addendum remains a separate later author act. A7 is validated evidence in review.

**D56 Update 13 — review addendum (2026-07-23): fresh Codex gpt-5.6-sol xHigh read-only evidence/provenance audit at exact N57 returned FINDINGS (six of seven points OK, one MINOR); the single bounded non-evidence correction applied; C57/F57 defined.** Reviewer: fresh Codex **gpt-5.6-sol** xHigh (session `019f8f03`, banner + rollout `model`/`effort` verified, read-only sandbox) over the complete `M56d..N57` diff including Notes. Verdict **FINDINGS**: topology (27/3/24), byte preservation vs the pinned table, manifests (differ only by the `job_metadata.txt` line), force-add + 12/11 tracked sets, Notes truthfulness, and frozen surfaces all **OK**; one **MINOR** — the recovery record §3 read "P6/P6b capture wrote only 10 files," which conflicts with the committed **11-file** original bundle (ten payloads plus the manifest; the manifest is 10 lines). The Opus 4.8 orchestrator independently confirmed the finding against source (`git ls-files` original == 11; original manifest == 10 lines) and agreed — no material Codex↔orchestrator disagreement, so GLM 5.2 was not consulted — and independently re-verified the six OK points (committed-blob SHA-256 for all 23 files == the pinned table; frozen surfaces `M56d..N57 -- experiments/ bistar_gp/ docs/ tests/` empty; record free of timing-magnitude interpretation). The one permitted bounded **non-evidence** correction reworded recovery-record §3 to "ten payload files … an 11-file bundle, missing `job_metadata.txt` … covering those ten payload files"; **no evidence-bundle file or manifest was touched** (the 23 immutable bundle files remain byte-identical to E57). The correction commit is **C57**; the corrected final head is **F57 = C57**. At F57 the 23 immutable bundle files equal E57; the recovery record at F57 differs from E57 (corrected) and is NOT claimed equal to E57. `M56d..F57` remains exactly the 27-file allowlist. This is an AI review, not a GitHub review or CI. Draft PR #30 remains **STOPPED before Ready**.

---

## D57: A7 measured-results addendum v1.23 — global Della thread pin 3 from job-11517022 timing evidence; poster milestone renumbered to D58; docs-only — 2026-07-23

**Milestone.** Docs-only implementation of the A7 measured-results / thread-pinning addendum authorized by prereg v1.2 point 6, the v1.5 owed-numbers paragraph, v1.6 item 6, v1.8 items 4–5, v1.9 item 1, v1.11 item 4, plan §1.3/§7 A7/risk R6, execution-protocol §8 allowlist C as amended by D56c, and the author ballot cast 2026-07-23. The read-only planning turn passed startup gate **5/5** at evidence-closure anchor `24f55c37`; extraction was performed by **ONE** fresh read-only Codex `gpt-5.6-sol` xHigh analyst (session `019f8f69`, model/effort verified from the rollout log), then independently cross-verified by the Fable orchestrator. There were zero material disagreements; GLM 5.2 was not consulted.

**Author ballot (cast 2026-07-23).** B1 ADOPTED WITH SCOPE CLARIFICATION: one global pin of threads = 3 for D19/A7-governed `bistar_gp` CPU workloads on the frozen `intel,cascade,rh9` 90-node Della pool, with the four environment variables, torch intra-op, and `cpus-per-task` consistently set to 3; this is not a claim about every conceivable future workload outside that scope. B2 ALTERNATIVE ADOPTED: no general 5% tie threshold is frozen from this single, unreplicated sweep; the 1.19% DERIVED sub-scale t3/t4 difference is insufficient here for a stable distinction, and t3 is chosen as the clear full-scale whole-cell winner because it avoids the t4 regression, uses fewer resources, minimizes combined sub+full elapsed, and supplies one operationally simple global pin; the single-run / no-replicate-variance caveat remains. B3 INCLUDE: core-seconds are informational only, not a second pin or alternative packing authority; deviation needs its own amendment. B4 OPTION (a): create D57 and renumber the prospective poster milestone from D57 to D58; no poster work begins.

**Decision (implementation).** Append prereg addendum v1.23 as planning/engineering evidence, never a scientific-paper result. Headline whole-cell elapsed values are **RECORDED seconds, rounded here only for display**: sub `34.59 / 24.87 / 24.47 / 24.18` and full `64.41 / 37.76 / 33.98 / 42.34` for threads 1/2/3/4; the full-t4 prior-evaluation regression is **DERIVED milliseconds** (`1000 x` recorded seconds), `17.85` versus `9.48` at t3. Adopt the scoped global pin 3. Disclose the post-run CWD-relative `job_metadata.txt` recovery and preserved incomplete bundle; the outcome-informed D56d amendment, original 14/16 rather than clean validation, and later amended 16/16 pass; both pre-cell SPENT attempts; one whole-cell run per matrix cell with no replicate variance; non-binding 600 s cell budgets and all statuses measured; arithmetically imposed 127 leapfrogs because the vehicle has no sampler; torch inter-op observed at 40 and never set; and zero failed prior evaluations plus two category-only DeprecationWarnings per cell. Recomputed projections all fit the frozen budgets; the binding row has **1.67x DERIVED headroom** under the row's own x1.5 engineering-overhead convention. The allowlist is exactly `docs/prereg-addenda-d19.md`, `Notes/DECISIONS.md`, `Notes/SCRATCHPAD.md`, and `Notes/CHATLOG.md`, on branch `docs/d19-v123-measured-results` off `24f55c37`.

**Commit topology.** The implementation head carries the complete four-file change. Reviewed head **R57** is the last non-Notes commit of the PR after the review gate. Merge anchor **M57** is the future true merge whose second parent equals the PR head; D57 deliberately does not name its own future merge SHA. The review gate is **ONE** fresh read-only Codex `gpt-5.6-sol` xHigh exact-head review of the complete four-file diff, with findings independently verified by the orchestrator; GLM 5.2 is used only for a material unresolved disagreement. At most **ONE** bounded docs-only correction pass is permitted, followed by narrow reconfirmation.

**Poster renumbering (B4).** The 2026-07-21 post-A7 roadmap directive prospectively labeled the poster milestone D57. That milestone is renumbered to **D58**; no poster work is begun.
