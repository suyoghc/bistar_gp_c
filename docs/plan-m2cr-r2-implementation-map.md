...
| 14 | Environment-freeze artifacts (Layer 0): exact frozen child-environment mapping; complete importable-artifact manifest over the four roots under the B15(ii) scope (normal `__pycache__` entries excluded; orphan, legacy, sourceless, importable-archive candidates included; catches an added `numpy/_distributor_init_local.py`); interpreter pin (version string plus resolved-target sha256, re-attested now, not inherited); pre-boundary attestation set; aggregating environment-freeze manifest carrying the single chain digest; and the native-stack expectations artifact (D48 Update 8: frozen native import list, v1.17 profile hash, torch/numpy build markers, Stage-B delta, loaded-image allowlist, measured expected loaded-image set, and — D48 Update 10 / finding 3 — the build-pinned bound sentinel `__hash__` value; infra-pinned as additional R2 Layer-0 content outside the four-member aggregator, and the derivation source for the eight mandatory attestation directives) (plan §3.1, §4.5.6, §4.5.7, §4.5.12; v1.19 §2, §5) | `bistar_gp/m2cr/environment_freeze.py` generators; committed artifacts under `docs/m2c_freeze/` | `tests/test_m2cr_environment_freeze.py`, `tests/test_m2cr_bootstrap.py` (directive enforcement), `tests/test_m2cr_capture.py` (derive/bind) |
...
| 20 | Full executable suite (plan §8) | — | `python -m pytest -q`; baseline 442 passed, 1 skipped recorded pre-edit; integrated tree 711 passed, 2 skipped; after the external-audit-round-3 remediation (F1–F6 + checkpoint CP-1..CP-5, head eeefeef) 723 passed, 2 skipped; after the F1/F2/F4 round-3 strengthening (head 8c24b1f) 730 passed, 2 skipped; after the Codex round-3 C1–C4 round-4 remediation and its five adjudicated delta-review rounds (final candidate 00c3a92) **774 passed, 2 skipped** (both remaining skips are the opt-in real-root walk and the pre-existing baseline skip) |
