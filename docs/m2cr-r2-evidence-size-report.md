...
Fixed-artifact total: **8,833,030 bytes** (measured; regenerated after the external audit
round-3 remediation — F3 worktree-relative closure pins and the F4 editable-filtered lock).
...
| Fixed freeze artifacts | 8,833,030 | measured (sum above) |
| Complete bundle, evidence classes with measured bases | 26,578,372 | derived: fixed total + both per-node products |
